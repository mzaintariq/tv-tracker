...
function expectBoundaryContrast(
  theme: ThemeTokens,
  foreground: string,
  background: string,
  minimum = 3,
...
describe("Phase 9D.4 visual accessibility contracts", () => {
  ...
  it("keeps focus-visible distinguishable with offset and a contrasting backup ring", () => {
    expect(css).toMatch(
      /:focus-visible\s*\{[\s\S]*outline:\s*3px solid var\(--background\)/,
    );
  ...
  it("keeps disabled controls identifiable without relying only on low opacity", () => {
    expect(css).toMatch(
      /\.interactive-control[\s\S]*:disabled[\s\S]*opacity:\s*1/,
    );
    expect(css).toMatch(
      /:disabled:not\(\.interactive-control\)[\s\S]*opacity:\s*1/,
    );
  ...
  it("disables non-essential progress interpolation under reduced motion", () => {
    expect(css).toMatch(
      /prefers-reduced-motion:\s*reduce[\s\S]*\.animate-pulse[\s\S]*animation:\s*none/,
    );
    expect(css).toMatch(
      /prefers-reduced-motion:\s*reduce[\s\S]*\.progress-fill[\s\S]*transition:\s*none/,
    );
  });
  ...
  it("preserves Phase 9D.2 names and Phase 9D.3 one-column narrow grids", () => {
    expect(
      readFileSync("src/components/shows/show-controls.tsx", "utf8"),
    ).toContain("fieldset");
    expect(
      readFileSync("src/components/import/import-controls.tsx", "utf8"),
    ).toContain("Manual TMDB ID");
    expect(
      readFileSync("src/components/explore/media-grid.tsx", "utf8"),
    ).toContain("min-[360px]:grid-cols-2");
    expect(
      readFileSync("src/components/shows/watch-list-sections.tsx", "utf8"),
  ...
  it.each([
    ...
    "meets contrast targets for touched %s theme tokens",
    (_name, theme) => {
