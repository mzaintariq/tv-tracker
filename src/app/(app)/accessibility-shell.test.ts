...
describe("Phase 9D.1 shell contracts", () => {
  ...
  it("defines visible focus, safe-area, and reduced-motion baselines", () => {
    ...
    expect(css).toMatch(
      /\.skip-link:focus-visible\s*\{[\s\S]*transform:\s*translateY\(0\)/,
    );
    expect(css).toMatch(
      /:focus-visible\s*\{[\s\S]*outline:\s*3px solid var\(--background\)/,
    );
    expect(css).toMatch(
      /:focus-visible\s*\{[\s\S]*box-shadow:\s*0 0 0 5px var\(--foreground\)/,
    );
    ...
    expect(css).toMatch(
      /--mobile-nav-clearance:[\s\S]*var\(--mobile-nav-height\)[\s\S]*var\(--safe-area-bottom\)/,
    );
    expect(css).toMatch(
      /\.app-main-content\s*\{[\s\S]*padding-bottom:\s*var\(--mobile-nav-clearance\)/,
    );
    expect(css).toMatch(
      /\.mobile-bottom-nav\s*\{[\s\S]*padding-bottom:\s*var\(--safe-area-bottom\)/,
    );
    ...
    expect(css).toMatch(
      /\[class\*="transition-"\][\s\S]*transition-duration:\s*0\.01ms/,
    );
  });
});
