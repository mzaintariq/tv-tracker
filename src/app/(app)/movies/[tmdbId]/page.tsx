...
export default async function MovieDetailPage({
  ...
  if (!movie) notFound();
  const date = movie.media.release_date
    ? new Intl.DateTimeFormat("en-US", {
        dateStyle: "long",
        timeZone: "UTC",
      }).format(new Date(`${movie.media.release_date}T00:00:00Z`))
      ...
      <header className="grid min-w-0 gap-6 sm:grid-cols-[180px_minmax(0,1fr)]">
        ...
        <div className="min-w-0 space-y-4">
          ...
          <p className="break-words">
            {movie.media.overview || "No overview is available."}
          </p>
          <p className="break-words font-medium">
            {movie.membership.watched_at ? "Watched" : "Watch Next"}
            {movie.membership.is_favourite ? " · Favourite" : ""}
          </p>
          <MovieControls
            tmdbId={tmdbId}
