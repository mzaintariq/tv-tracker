  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-8">
      ...
      {data.staleTmdbIds.length ? (
        <UpcomingRefreshCoordinator tmdbIds={data.staleTmdbIds} />
      ) : null}
      {!data.trackedShowCount ? (
        <div className="rounded-xl border border-dashed border-[var(--border)] p-8 text-center">
          <h2 className="text-xl font-semibold">No active shows yet</h2>
          <p className="mt-2 text-[var(--muted)]">
            Add a show in Explore to see its announced releases.
          </p>
          <Link
        ...
        <UpcomingSections groups={data.groups} today={data.today} />
      ) : (
        <div className="rounded-xl border border-dashed border-[var(--border)] p-8 text-center">
          ...
          <p className="mt-2 text-[var(--muted)]">
            Newly announced episodes will appear here after metadata updates.
          </p>
        </div>
      )}
    </div>
