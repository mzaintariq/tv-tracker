...
import {
  ...
  ShowTrailerSection,
} from "@/components/shows/show-detail-sections";
...
export default async function ShowDetailPage({
  ...
  const certificationPromise = region
    ? loadTvRegionalCertification(tmdbId, region)
    : null;
  const certification = certificationPromise
    ? await certificationPromise.catch(() => null)
    : null;
    ...
    <article className="mx-auto w-full min-w-0 max-w-5xl space-y-6 sm:space-y-8">
      <header className="grid min-w-0 gap-4 sm:grid-cols-[180px_minmax(0,1fr)]">
        <div className="relative mx-auto aspect-[2/3] w-full max-w-[180px] overflow-hidden rounded-xl bg-[var(--surface-elevated)] sm:mx-0">
          <MediaPoster
            source={detail.media.poster_path}
        ...
        <div className="min-w-0 space-y-2 sm:space-y-4">
          <div className="min-w-0">
            ...
            <p className="break-words text-[var(--muted)] text-sm sm:text-base">
              {formatDate(detail.media.release_date) ??
                "First air date unknown"}
              {detail.media.average_episode_runtime_minutes
                ? ` · ${detail.media.average_episode_runtime_minutes} min average`
                : ""}
          ...
          <div className="min-w-0 text-sm flex flex-wrap items-center gap-2 sm:flex-row sm:gap-3">
            <Suspense fallback={null}>
              <ShowTrailerSection promise={trailerPromise} />
            </Suspense>
            {detail.media.vote_average !== null ? (
              <p
                className="text-xs sm:text-sm"
                aria-label={`TMDB rating ${detail.media.vote_average.toFixed(1)} out of 10${detail.media.vote_count !== null ? ` from ${detail.media.vote_count} votes` : ""}`}
              >
                <span className="font-semibold">TMDB</span>{" "}
                {detail.media.vote_average.toFixed(1)} / 10
                {detail.media.vote_count !== null
                  ? ` · ${formatVoteCount(detail.media.vote_count)} votes`
                  : ""}
              </p>
            ) : null}
          </div>
          {networks.length || statusSummary ? (
            <div className="flex min-w-0 flex-wrap items-center gap-2 text-xs sm:gap-3 sm:text-sm">
              {networks.length ? (
          ...
          {detail.membership ? <ProgressBar progress={progress} /> : null}
          {loaded.syncError ? (
            <p role="alert" className="break-words text-[var(--danger)]">
              {loaded.syncError}
            </p>
          ) : null}
        </div>
      ...
      {detail.membership ? (
        <section
          aria-label="Show tracking actions"
          className="relative min-w-0 space-y-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-3 sm:p-6"
        >
          <SettingsControls
            tmdbId={tmdbId}
...
function Overview({
  ...
  const creators = parseNamedFacts(media.creators),
    countries = parseCountries(media.origin_countries);
  const regionName = regionDisplayName(region),
    regionLabel = region ? `${regionName ?? region} (${region})` : null;
  const castPromise = loadTvTopCast(tmdbId),
    providerPromise = region
      ? loadRegionalWatchProviders("tv", tmdbId, region)
      : null,
    linksPromise = loadTvExternalLinks(tmdbId);
  ...
  return (
    <div className="min-w-0 space-y-6 sm:space-y-8">
      <div className="space-y-1 sm:space-y-2">
        <h2 className="text-lg sm:text-xl font-semibold">Show facts</h2>
        <section className={sectionClass}>
          <dl className="grid min-w-0 grid-cols-2 gap-4 sm:grid-cols-3">
            <Fact label="Status" value={showStatusSummary(media)} />
            <Fact
              label="First air date"
              value={formatDate(media.release_date)}
            />
            <Fact
              label="Last air date"
              value={formatDate(media.last_air_date)}
            />
            <Fact
              label="Average runtime"
              value={
                media.average_episode_runtime_minutes
                  ? `${media.average_episode_runtime_minutes} min`
                  : null
              }
            />
            <Fact
              label="Language"
              value={languageDisplayName(media.original_language)}
            />
            <Fact label="Certification" value={certification} />
            <Fact
      ...
      <Suspense
        fallback={<OptionalShowSectionSkeleton label="Where to watch" />}
      >
        <ShowProvidersSection
      ...
      <Suspense
        fallback={<OptionalShowSectionSkeleton label="Cast and creators" />}
      >
        <ShowCreditsSection creators={creators} promise={castPromise} />
      </Suspense>
      <div className="space-y-1 sm:space-y-2">
...
function Episodes({
  ...
  const defaultOpenSeason = defaultOpenRegularSeason(
    seasons,
    watchedIds,
    today,
  );
  return (
    <div className="min-w-0 space-y-4 sm:space-y-6">
      {!detail.membership ? (
        <InitialProgressForm tmdbId={tmdbId} episodes={detail.episodes} />
      ) : null}
      {detail.episodes.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[var(--border)] p-6 text-center text-sm text-[var(--muted)] sm:p-8 sm:text-base">
          No episode metadata is available yet.
        </div>
      ) : (
        [...seasons.entries()].map(([season, episodes]) => {
          ...
          return (
            <details
              key={season}
              open={season === defaultOpenSeason}
              className="group min-w-0 rounded-xl border border-[var(--border)] bg-[var(--surface)]"
            >
              <summary className="interactive-control touch-target grid min-w-0 cursor-pointer list-none grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-xl px-3 py-2 marker:content-none sm:gap-3 sm:px-4 sm:py-3 [&::-webkit-details-marker]:hidden">
                <div className="min-w-0">
                  <h2 className="break-words text-lg font-semibold sm:text-xl">
                    {season === 0 ? "Specials (Season 0)" : `Season ${season}`}
                  </h2>
                  <p className="break-words text-xs text-[var(--muted)] sm:text-sm">
                    {watchedCount} of {episodes.length} watched
                    {season === 0 ? " · Excluded from normal progress" : ""}
                  </p>
                </div>
                <span
                  aria-hidden="true"
                  className="shrink-0 text-lg group-open:rotate-90 sm:text-xl"
                >
                  ›
                </span>
              </summary>
              <div className="min-w-0 space-y-2 border-t border-[var(--border)] p-3 sm:space-y-3 sm:p-4">
                {detail.membership ? (
                  <SeasonControls
                    tmdbId={tmdbId}
                    mediaId={detail.media.id}
                ...
                <ol className="min-w-0 overflow-hidden divide-y divide-[var(--border)] rounded-xl border border-[var(--border)]">
                  {episodes.map((episode) => {
                    const isWatched = watchedIds.has(episode.id);
                    return (
                      <li
                        key={episode.id}
                        className={`min-w-0 space-y-2 p-3 sm:space-y-3 sm:p-4 ${isWatched ? "bg-[color-mix(in_srgb,var(--success)_9%,var(--surface))]" : ""}`}
                      >
                        <div className="min-w-0">
                          <h3 className="flex min-w-0 items-start justify-between gap-2 text-sm font-semibold sm:text-base">
                            <span className="min-w-0 break-words">
                              S{String(episode.season_number).padStart(2, "0")} |
                              E{String(episode.episode_number).padStart(2, "0")} —{" "}
                              {episode.title}
                            </span>
                            {isWatched ? (
                              <span className="inline-flex shrink-0 rounded-full border border-[var(--success)] px-2 py-0.5 text-xs text-[var(--success)]">
                        ...
                        {detail.membership ? (
                          <EpisodeControls
                            ...
                            mediaId={detail.media.id}
                            episode={episode}
                            watched={watchedMap.get(episode.id)}
                            today={today}
                            timeZone={timeZone}
                          />
                        ) : null}
