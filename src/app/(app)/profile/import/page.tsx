...
export default async function ImportPage() {
  ...
  const result = await supabase
    .from("imports")
    .select("id,status,created_at,total_items,applied_items,skipped_items")
    .eq("user_id", user.id)
    ...
    <div className="min-w-0 space-y-8">
      <header className="min-w-0">
        ...
        <p className="break-words text-[var(--muted)]">
          Analyze your GDPR ZIP without storing the raw archive. Timezone-less
      ...
      <section className="min-w-0 space-y-3">
        ...
        {imports.map((item) => (
          <Link
            className="block min-w-0 rounded-xl border p-4"
            key={item.id}
            href={`/profile/import/${item.id}`}
          >
            <span className="block break-words font-medium">
              {new Date(item.created_at).toLocaleString()}
            </span>
            <span className="mt-1 block break-words text-[var(--muted)]">
              {item.status} · {item.applied_items}/{item.total_items} applied ·{" "}
              {item.skipped_items} skipped
            </span>
          </Link>
        ))}
        {!imports.length ? (
          <p className="text-[var(--muted)]">No imports analyzed yet.</p>
        ) : null}
      </section>
      <section className="min-w-0 rounded-xl border p-4 sm:p-5">
        <h2 className="break-words text-xl font-semibold">Privacy cleanup</h2>
        <p className="my-2 break-words text-sm text-[var(--muted)]">
          Forgetting import data keeps shows, movies, favourites, and watch
          history already imported into TrackTV.
        </p>
        <ForgetTvTimeButton />
      </section>
    </div>
