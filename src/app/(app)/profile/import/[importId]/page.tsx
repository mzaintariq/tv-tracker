...
import {
  DeleteImportButton,
  MatchMoreButton,
} from "@/components/import/import-controls";
...
import {
  requireImportProgress,
  requireImportSession,
} from "@/lib/import/tv-time/route-results";
...
export default async function ImportDetailPage({
  ...
  if (!session) notFound();
  const [progressResult, matchingProgress, resolutionSnapshot] =
    await Promise.all([
      supabase.rpc("get_tv_time_import_apply_progress", {
        p_import_id: importId,
      }),
      loadMatchingProgress(supabase, importId, user.id),
      loadResolutionSnapshot(supabase, importId, user.id),
    ]);
  const progress = requireImportProgress(
    progressResult,
  ) as unknown as ImportApplyProgress | null;
  return (
    <div className="min-w-0 space-y-8">
      <header className="min-w-0">
        ...
        <p className="break-words text-sm text-[var(--muted)]">
          Legacy Unix watch_date values are authoritative. Other timezone-less
          created_at values are interpreted as UTC; historical local timezone is
          not known. Existing TrackTV watched dates are preserved.
        </p>
      </header>
      <section className="grid min-w-0 grid-cols-1 gap-3 min-[360px]:grid-cols-2 sm:grid-cols-4">
        <div className="min-w-0 rounded-xl border p-4">
          <span className="break-words">{session.total_items}</span>
          <br />
          items
        </div>
        <div className="min-w-0 rounded-xl border p-4">
          <span className="break-words">{session.matched_items}</span>
          <br />
          matched
        </div>
        <div className="min-w-0 rounded-xl border p-4">
          <span className="break-words">{session.applied_items}</span>
          <br />
          applied
        </div>
        <div className="min-w-0 rounded-xl border p-4">
          <span className="break-words">{session.skipped_items}</span>
          <br />
          skipped
        </div>
      </section>
      {["matching", "awaiting_resolution"].includes(session.status) ? (
        <MatchMoreButton
          importId={importId}
          autoStart={shouldAutoStartMatching(
            session.status,
            matchingProgress?.remaining ?? 0,
          )}
        />
      ) : null}
      {matchingProgress ? (
        <ImportMatchingProgress
          importId={importId}
          initialProgress={matchingProgress}
        />
      ) : null}
      {resolutionSnapshot ? (
        <ImportResolutionWorkspace
          importId={importId}
          initialItems={resolutionSnapshot.items}
          initialIssues={resolutionSnapshot.issues}
          initialRevision={resolutionSnapshot.revision}
        />
      ) : null}
      {["ready", "applying", "paused"].includes(session.status) && progress ? (
        <section className="min-w-0 rounded-xl border p-4 sm:p-5">
          <h2 className="mb-3 break-words text-xl font-semibold">
            Apply or resume
          </h2>
          <ImportUploadForm
            mode="apply"
