...
const mocks = vi.hoisted(() => {
  class MockExportLoadError extends Error {
    ...
    constructor(stage: "watched_history") {
  ...
  return {
    createClient: vi.fn(),
    loadExportSourceData: vi.fn(),
    ExportLoadError: MockExportLoadError,
  };
});
vi.mock("@/lib/supabase/server", () => ({ createClient: mocks.createClient }));
vi.mock("@/lib/export/data", () => ({
  ExportLoadError: mocks.ExportLoadError,
  loadExportSourceData: mocks.loadExportSourceData,
}));
...
describe("GET /api/export", () => {
  beforeEach(() => {
    vi.useFakeTimers();
    vi.setSystemTime(new Date("2026-07-17T10:30:45.123Z"));
    mocks.createClient.mockReset();
    mocks.loadExportSourceData.mockReset();
  });
  afterEach(() => {
    vi.useRealTimers();
    vi.restoreAllMocks();
  ...
  it("returns a sanitized private error when authentication infrastructure fails", async () => {
    ...
    expect(response.headers.get("cache-control")).toBe("private, no-store");
    expect(await response.json()).toEqual({
      error: "Your data could not be exported.",
    });
    expect(console.error).toHaveBeenCalledWith(
      JSON.stringify({
        event: "user_export_failed",
        stage: "auth",
  ...
  it("returns the private versioned download with a safe filename", async () => {
    const client = {
      auth: {
        getUser: vi
          ...
          .mockResolvedValue({
            data: {
              user: { id: "owner-id", email: "private@example.invalid" },
            },
          }),
      },
    };
    mocks.createClient.mockResolvedValue(client);
    mocks.loadExportSourceData.mockResolvedValue(source);
    const response = await GET();
    ...
    expect(response.headers.get("content-disposition")).toBe(
      'attachment; filename="tracktv-export-20260717T103045Z.json"',
    );
    expect(response.headers.get("cache-control")).toBe("private, no-store");
    expect(response.headers.get("x-content-type-options")).toBe("nosniff");
    ...
    expect(document).toMatchObject({
      schema: "tracktv.user-export",
      version: 1,
      generatedAt: "2026-07-17T10:30:45.123Z",
      tvShows: [],
      movies: [],
    });
  ...
  it("returns a safe generic error without partial content or download headers", async () => {
    ...
    mocks.createClient.mockResolvedValue({
      auth: {
        getUser: vi
          .fn()
          .mockResolvedValue({ data: { user: { id: "owner-id" } } }),
      },
    });
    mocks.loadExportSourceData.mockRejectedValue(
      new ExportLoadError("watched_history"),
    );
    const response = await GET();
    expect(response.status).toBe(500);
    expect(response.headers.get("content-disposition")).toBeNull();
