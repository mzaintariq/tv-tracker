...
export async function POST(
  ...
  try {
    ...
    const session = await supabase
      ...
      .eq("user_id", user.id)
      .single();
    if (session.error || session.data.source_fingerprint !== fingerprint)
      return NextResponse.json(
        {
          error: "Choose the same ZIP that was analyzed.",
          code: "fingerprint_mismatch",
        },
        { status: 409 },
      );
    const result = await applyNormalizedImport(
      user.id,
      importId,
      normalizeTvTimeExport(readAllowedTvTimeZip(bytes)),
    );
    ...
    await admin
      .from("imports")
      .update({ status: "paused", last_error_code: failure.code })
      .eq("id", importId)
      .eq("user_id", user.id)
      .in("status", ["applying", "paused"]);
    const [tvApplied, tvPending, movieApplied, moviePending] =
      await Promise.all([
        admin
          .from("import_items")
    ...
    console.error(
      JSON.stringify({
        event: "tv_time_apply_paused",
        correlationId: crypto.randomUUID(),
        phase: diagnostic?.stage.startsWith("movie_") ? "movie" : "tv",
        stage: diagnostic?.stage ?? "unknown",
        category: diagnostic?.category ?? "unknown_error",
        safeCode: diagnostic?.safeCode ?? null,
        retryable: diagnostic?.retryable ?? false,
        retryAttempts: 0,
        progress: {
          tvApplied: tvApplied.count ?? 0,
          tvRemaining: tvPending.count ?? 0,
          movieApplied: movieApplied.count ?? 0,
          movieRemaining: moviePending.count ?? 0,
        },
      }),
    );
    return NextResponse.json(
