...
import {
  TvTimeImportDatabaseError,
  TvTimeImportError,
} from "@/lib/import/tv-time/errors";
...
function logAnalyzeFailure(stage: AnalyzeStage, code: string): void {
  console.error(
    JSON.stringify({ event: "tv_time_analyze_failed", stage, code }),
  );
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (error instanceof TvTimeImportError) {
      const errorStage: AnalyzeStage = error.code.startsWith("csv_")
        ? "csv_schema"
        : stage;
    ...
    if (error instanceof TvTimeImportDatabaseError) {
      ...
      return NextResponse.json(
        { error: "The import could not be analyzed.", code: error.code },
        { status: 500 },
      );
    }
    ...
    return NextResponse.json(
      { error: "The import could not be analyzed.", code },
      { status: 500 },
    );
  }
}
