...
describe("Explore library reads", () => {
  ...
  it("handles show-only, movie-only, and combined memberships", () => {
    expect(buildLibraryKeys([show], [], [media[0]])).toEqual(
      new Set(["tv:10"]),
    );
    expect(buildLibraryKeys([], [movie], [media[1]])).toEqual(
      new Set(["movie:20"]),
    );
    expect(buildLibraryKeys([show], [movie], media)).toEqual(
      new Set(["tv:10", "movie:20"]),
    );
  });
  ...
  it("paginates memberships instead of accepting a truncated first page", async () => {
    const first = Array.from(
      { length: EXPLORE_LIBRARY_PAGE_SIZE },
      (_, index) => ({ media_item_id: `id-${index}` }),
    );
    const fetchPage = vi
      .fn()
      .mockResolvedValueOnce({ data: first, error: null })
      .mockResolvedValueOnce({
        data: [{ media_item_id: "last" }],
        error: null,
      });
    ...
    expect(fetchPage).toHaveBeenNthCalledWith(
      2,
      EXPLORE_LIBRARY_PAGE_SIZE,
      EXPLORE_LIBRARY_PAGE_SIZE * 2 - 1,
    );
  });

  it.each(["user_shows", "user_movies", "media_items"] as const)(
    "keeps a genuine %s failure visible",
    (stage) => {
      expect(() =>
        requireLibraryRows(
          { data: null, error: { message: "private raw error" } },
          stage,
        ),
      ).toThrow(ExploreLibraryReadError);
      try {
        requireLibraryRows({ data: null, error: {} }, stage);
      } catch (error) {
        expect((error as ExploreLibraryReadError).stage).toBe(stage);
      }
    },
  );
  ...
  it("rejects missing required media instead of corrupting membership indicators", () => {
    expect(() => buildLibraryKeys([show], [movie], [media[0]])).toThrow(
      ExploreLibraryReadError,
    );
  });
