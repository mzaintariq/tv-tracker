...
export function isReleasedRegularEpisode(
  episode: Pick<Episode, "season_number" | "air_date">,
  today: string,
): boolean {
  return (
...
export function calculateShowProgress(
  ...
  const released = episodes.filter((episode) =>
    isReleasedRegularEpisode(episode, today),
  );
  const watched = released.reduce(
    (count, episode) => count + (watchedEpisodeIds.has(episode.id) ? 1 : 0),
    0,
  );
  ...
  return {
    ...
    total,
    percentage,
    state: isShowEnded(tmdbStatus) ? "complete" : "caught-up",
  };
}
