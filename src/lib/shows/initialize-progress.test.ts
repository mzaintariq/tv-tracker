...
describe("initial show submission", () => {
  it("submits Start from Season 1, Episode 1 without episode synchronization when shared metadata exists", async () => {
    const synchronize = vi
      .fn()
      .mockRejectedValue(new Error("episode synchronization unavailable"));
    ...
    ).resolves.toEqual({ failedSeasons: [] });
  ...
  it("surfaces membership initialization failure", async () => {
    const initializeMembership = vi
      .fn()
      .mockResolvedValue({ error: "Could not add show." });
    await expect(
      initializeProgress(
        { mode: "start" },
        "cached-media-id",
    ...
    ).resolves.toEqual({ error: "Could not add show." });
  });
});
