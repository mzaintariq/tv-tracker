...
const CUTOFF = new Date(
  new Date(NOW).getTime() - INACTIVITY_THRESHOLD_DAYS * 86_400_000,
);
...
function snapshot(
  options: {
    ...
    episodes?: Array<
      Pick<Episode, "id" | "season_number" | "episode_number" | "air_date">
    >;
  ...
  return {
    ...
    media: {
      ...
      tmdb_status: options.tmdbStatus ?? "Returning Series",
      last_synced_at: NOW,
      created_at: NOW,
      updated_at: NOW,
    } as MediaItem,
    episodes: episodes.map((episode) => ({
      ...
      title: episode.id,
      runtime_minutes: null,
      tmdb_episode_id: episode.episode_number,
      last_synced_at: NOW,
      created_at: NOW,
      updated_at: NOW,
    })) as Episode[],
    watched: (options.watched ?? []).map((row, index) => ({
      id: `watched-${title}-${index}`,
      user_id: "user",
      episode_id: row.episodeId,
      watched_at: row.watchedAt,
      created_at: row.watchedAt,
      updated_at: row.watchedAt,
    })) as WatchedEpisode[],
  };
}
...
describe("watch list derivation", () => {
  ...
  it("advances Watch Next after the current episode is marked watched", () => {
    ...
    const before = snapshot({
      episodes,
      watched: [{ episodeId: "one", watchedAt: "2026-07-01T00:00:00Z" }],
    });
    ...
    expect(deriveWatchList([before], TODAY, NOW).watchNext[0].episode.id).toBe(
      "two",
    );
    expect(deriveWatchList([after], TODAY, NOW).watchNext[0].episode.id).toBe(
      "three",
    );
  });
  ...
  it("puts a recently watched unfinished show only in Watch Next", () => {
    ...
    expect(result.watchNext.map((item) => item.media.title)).toEqual([
      "Recent",
    ]);
  ...
  it("promotes a stale show with a newly aired unwatched episode into Watch Next only", () => {
    ...
    const result = deriveWatchList(
      [partial("Renewed", watchedAt, "2026-07-10")],
      TODAY,
      NOW,
    );
    expect(result.watchNext.map((item) => item.media.title)).toEqual([
  ...
  it("ignores Season 0 watching and air dates for Watch Next eligibility", () => {
    ...
    const result = deriveWatchList([show], TODAY, NOW);
    expect(result.watchNext).toHaveLength(0);
    expect(result.inactive.map((item) => item.media.title)).toEqual([
      "Specials",
    ]);
  });
  ...
  it("treats exactly 30 days ago as recent for Watch Next", () => {
    ...
    expect(
      deriveWatchList([partial("Exact", atCutoff)], TODAY, NOW).watchNext,
    ).toHaveLength(1);
    expect(
      deriveWatchList([partial("Exact", atCutoff)], TODAY, NOW).inactive,
    ).toHaveLength(0);
    expect(
      deriveWatchList([partial("Older", older)], TODAY, NOW).watchNext,
    ).toHaveLength(0);
    expect(
      deriveWatchList([partial("Older", older)], TODAY, NOW).inactive,
    ).toHaveLength(1);
    expect(
      deriveWatchList([partial("Newer", newer)], TODAY, NOW).watchNext,
    ).toHaveLength(1);
    expect(
      deriveWatchList([partial("Newer", newer)], TODAY, NOW).inactive,
    ).toHaveLength(0);
  });
  ...
  it("never duplicates memberships across Watch Next and Haven't watched for a while", () => {
    ...
    const watchNextIds = new Set(
      result.watchNext.map((item) => item.membership.id),
    );
    const inactiveIds = new Set(
      result.inactive.map((item) => item.membership.id),
    );
    for (const id of watchNextIds) expect(inactiveIds.has(id)).toBe(false);
    expect(result.watchNext.map((item) => item.media.title).sort()).toEqual([
      "Aired",
      "Recent",
    ]);
    expect(result.inactive.map((item) => item.media.title)).toEqual(["Stale"]);
  ...
  it("derives caught up, completed, and ended-incomplete correctly", () => {
    const watched = [
      { episodeId: "Example-e1", watchedAt: "2026-01-01T00:00:00Z" },
    ];
    const caught = deriveWatchList([snapshot({ watched })], TODAY, NOW);
    const completed = deriveWatchList(
      [snapshot({ watched, tmdbStatus: "Ended" })],
      TODAY,
      NOW,
    );
    const incomplete = deriveWatchList(
      [
        snapshot({
          tmdbStatus: "Ended",
          episodes: [
  ...
  it("keeps caught-up and completed shows out of Watch Next and inactive", () => {
    const watched = [
      { episodeId: "Example-e1", watchedAt: "2026-07-10T00:00:00Z" },
    ];
    const caught = deriveWatchList([snapshot({ watched })], TODAY, NOW);
    const completed = deriveWatchList(
      [snapshot({ watched, tmdbStatus: "Ended" })],
  ...
  it("makes paused and dropped primary and excludes them from overlays", () => {
    ...
    const result = deriveWatchList(
      [
        snapshot({ title: "Paused", status: "paused", episodes, watched }),
        snapshot({ title: "Dropped", status: "dropped", episodes, watched }),
      ],
      TODAY,
      NOW,
    );
  ...
  it("assigns exactly one primary state to every tracked show", () => {
    const result = deriveWatchList(
      [
        ...
        snapshot({
          title: "Caught",
          watched: [{ episodeId: "Caught-e1", watchedAt: NOW }],
        }),
        snapshot({
          title: "Complete",
          tmdbStatus: "Ended",
          watched: [{ episodeId: "Complete-e1", watchedAt: NOW }],
        }),
        snapshot({
          title: "Partial",
          episodes: [
  ...
  it("sorts every section deterministically", () => {
    ...
    expect(result.notStarted.map((item) => item.media.title)).toEqual([
      "Newer",
      "Older",
    ]);
    expect(result.recentlyWatched.map((item) => item.media.title)).toEqual([
      "Recent",
      "Alpha",
      "Beta",
    ]);
  ...
  it("sorts inactivity oldest-first and title-sorts primary sections", () => {
    ...
    expect(result.paused.map((item) => item.media.title)).toEqual([
      "Alpha",
      "Zulu",
    ]);
  });
  ...
  it("limits Recently Watched", () => {
    const show = snapshot({
      episodes: Array.from(
        { length: RECENTLY_WATCHED_LIMIT + 2 },
        (_, index) => ({
          id: `e${index}`,
          season_number: 1,
      ...
      watched: Array.from(
        ...
        (_, index) => ({
          episodeId: `e${index}`,
          watchedAt: `2026-07-${String(index + 1).padStart(2, "0")}T00:00:00Z`,
        }),
      ),
    });
    expect(deriveWatchList([show], TODAY, NOW).recentlyWatched).toHaveLength(
      RECENTLY_WATCHED_LIMIT,
    );
  });
});
