...
import {
  EPISODE_METADATA_STALE_HOURS,
  isEpisodeMetadataStale,
...
describe("Upcoming derivation", () => {
  it("includes the previous two days, today, and future regular episodes", () => {
    ...
    expect(result.map((group) => group.airDate)).toEqual([
      "2026-07-14",
      TODAY,
      "2026-07-16",
    ]);
    expect(
      result
        ...
        .map((item) => (item.kind === "episode" ? item.episode.id : "group")),
    ).toEqual(["past", "today", "future"]);
  });

  it("excludes paused and dropped shows", () => {
    const episode = [
      { id: "episode", season_number: 1, episode_number: 1, air_date: TODAY },
    ];
    expect(
      deriveUpcoming(
        [
          snapshot("Paused", "paused", episode),
          snapshot("Dropped", "dropped", episode),
        ],
        TODAY,
      ),
    ).toEqual([{ airDate: TODAY, items: [] }]);
  });
  ...
  it("groups only multiple episodes from the same show, season, and date", () => {
    const result = deriveUpcoming(
      [
        ...
        snapshot("Beta", "active", [
          { id: "b1", season_number: 1, episode_number: 1, air_date: TODAY },
        ]),
      ],
      TODAY,
    );
    expect(
      result[0].items.map((item) => [item.media.title, item.kind]),
    ).toEqual([
      ["Alpha", "season"],
      ["Alpha", "episode"],
      ["Beta", "episode"],
    ...
    expect(
      grouped.kind === "season"
        ? grouped.episodes.map((episode) => episode.id)
        : [],
    ).toEqual(["a1", "a2"]);
  });
  ...
  it("sorts dates and same-date items deterministically", () => {
    const result = deriveUpcoming(
      [
        ...
        snapshot("Alpha", "active", [
          { id: "a", season_number: 1, episode_number: 3, air_date: TODAY },
        ]),
      ],
      TODAY,
    );
...
describe("Upcoming calendar helpers", () => {
  ...
  it("labels Yesterday, Today, Tomorrow, and later dates", () => {
    expect(upcomingDateLabel(addCalendarDays(TODAY, -1), TODAY)).toBe(
      "Yesterday",
    );
...
describe("episode metadata freshness", () => {
  it("treats null, invalid, and exactly 24-hour-old timestamps as stale", () => {
    ...
    const cutoff = new Date(
      NOW.getTime() - EPISODE_METADATA_STALE_HOURS * 60 * 60 * 1000,
    );
    expect(isEpisodeMetadataStale(cutoff.toISOString(), NOW)).toBe(true);
    expect(
      isEpisodeMetadataStale(new Date(cutoff.getTime() + 1).toISOString(), NOW),
    ).toBe(false);
  });
  ...
  it("refreshes only stale active shows under the existing ended interpretation", () => {
    ...
    expect(
      shouldAutomaticallyRefreshEpisodes("active", "Ended", null, NOW),
    ).toBe(false);
    expect(
      shouldAutomaticallyRefreshEpisodes("active", "Canceled", null, NOW),
    ).toBe(true);
  });
