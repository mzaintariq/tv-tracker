  ...
  episodes: [
    {
      ...
      season_number: 1,
      episode_number: 1,
    } as ShowDetailData["episodes"][number],
  ],
  watched: [],
});
...
describe("loadShowPageData", () => {
  it("loads a valid uncached TMDB show for the setup screen", async () => {
    ...
    await expect(
      loadShowPageData("user", 254528, {
        load,
        synchronize: vi.fn().mockResolvedValue({}),
      }),
    ).resolves.toMatchObject({ detail: resolved, tmdbNotFound: false });
  });

  it("uses a locally cached show with episodes", async () => {
    const synchronize = vi.fn();
    await loadShowPageData("user", 254528, {
  ...
  it("preserves existing library membership", async () => {
    ...
    const result = await loadShowPageData("user", 254528, {
      load: vi.fn().mockResolvedValue(detail(membership)),
      synchronize: vi.fn(),
    });
  ...
  it("recovers a metadata row written during failed prefetch synchronization", async () => {
    ...
    const result = await loadShowPageData("user", 254528, {
      load,
      synchronize: vi.fn().mockRejectedValue(new Error("episodes failed")),
    });
  ...
  it("returns not found for a genuine TMDB 404", async () => {
    const result = await loadShowPageData("user", 999999, {
      load: vi.fn().mockResolvedValue(null),
      synchronize: vi.fn().mockRejectedValue(new TmdbApiError("missing", 404)),
    });
  ...
  it("does not disguise metadata write failures as not found", async () => {
    ...
    await expect(
      loadShowPageData("user", 254528, {
        load: vi.fn().mockResolvedValue(null),
        synchronize: vi.fn().mockRejectedValue(failure),
      }),
