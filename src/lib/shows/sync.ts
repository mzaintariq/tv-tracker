...
export async function synchronizeShow(
  tmdbId: number,
  forceRefresh = false,
): Promise<SyncResult> {
  const details = await getTvDetails(tmdbId, forceRefresh);
  const admin = createAdminClient();
  const { data: media, error: mediaError } = await admin
    .from("media_items")
    .upsert(mapTmdbTvDetailsToCacheRow(details), {
      onConflict: "tmdb_id,media_type",
    })
    .select("id")
    .single();
  if (mediaError || !media)
    throw new Error(mediaError?.message ?? "Could not cache show metadata.");
  const seasons = (details.seasons ?? [])
    .map((season) => season.season_number)
    .filter((number) => Number.isInteger(number) && number >= 0);
  const settled = await settleWithConcurrency(seasons, CONCURRENCY, (season) =>
    getTvSeason(tmdbId, season, forceRefresh),
  );
  const failedSeasons: number[] = [];
  const synchronizedSeasons: number[] = [];
  const regularRows: EpisodeInsert[] = [];
  const specialRows: EpisodeInsert[] = [];
  ...
  settled.forEach((result, index) => {
    ...
    synchronizedSeasons.push(season);
    for (const episode of result.value.episodes) {
      const row = mapTmdbEpisode(media.id, episode, now);
      if (row) (season === 0 ? specialRows : regularRows).push(row);
    }
  });
  const regularResult = await admin.rpc("reconcile_show_episodes", {
    p_media_item_id: media.id,
    p_episodes: reconciliationPayload(regularRows),
  });
  if (regularResult.error) throw new ShowEpisodeReconciliationError();
  try {
    const specialResult = await admin.rpc("reconcile_show_episodes", {
      p_media_item_id: media.id,
      p_episodes: reconciliationPayload(specialRows),
    });
