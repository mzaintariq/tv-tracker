...
describe("calendar dates and timestamp instants", () => {
  ...
  it("derives today in positive and negative configured UTC offsets", () => {
    ...
    expect(dateInTimeZone(crossingMidnight, "America/Los_Angeles")).toBe(
      "2026-07-14",
    );
  });
  ...
  it("parses date-only values without treating them as timestamp instants", () => {
    expect(parseDateOnly("2026-03-08")).toEqual({
      year: 2026,
      month: 3,
      day: 8,
    });
    ...
    expect(
      formatDateOnly("2026-07-15", { month: "long", day: "numeric" }),
    ).toBe("July 15");
  });
  ...
  it("formats watched timestamps in the configured timezone while preserving the instant", () => {
    ...
    expect(formatTimestamp(timestamp, "Asia/Karachi")).toContain(
      "Jul 15, 2026",
    );
    expect(formatTimestamp(timestamp, "America/Los_Angeles")).toContain(
      "Jul 14, 2026",
    );
  });
  ...
  it("round-trips datetime-local values through the configured timezone", () => {
    expect(
      timestampToDateTimeLocal("2026-01-02T03:04:00.000Z", "Asia/Karachi"),
    ).toBe("2026-01-02T08:04");
    expect(dateTimeLocalToTimestamp("2026-01-02T08:04", "Asia/Karachi")).toBe(
    ...
    ).toBe("2026-07-14T18:00");
    expect(
      dateTimeLocalToTimestamp("2026-07-14T18:00", "America/Los_Angeles"),
    ).toBe("2026-07-15T01:00:00.000Z");
  });

  it("rejects nonexistent DST wall times", () => {
    expect(
      dateTimeLocalToTimestamp("2026-03-08T02:30", "America/Los_Angeles"),
    ).toBeNull();
  });
});
