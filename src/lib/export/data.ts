...
export class ExportLoadError extends Error {
  ...
  constructor(stage: ExportLoadStage) {
    super("Export data could not be loaded.");
...
async function atStage<T>(
  stage: ExportLoadStage,
  work: () => Promise<T>,
): Promise<T> {
  try {
    return await work();
  } catch {
    throw new ExportLoadError(stage);
  }
}
  ...
  const pages = await mapWithConcurrency(
    ...
    async (ids) => {
      const result = await client
        .from("episodes")
        .select(
          "id,media_item_id,tmdb_episode_id,season_number,episode_number,title,air_date",
        )
        .in("id", ids)
...
export async function loadExportSourceData(
  client: ExportClient,
  userId: string,
  ...
  const profile = await atStage("profile", async () => {
    const result = await client
      .from("profiles")
      .select("display_name,avatar_url,timezone,theme")
      .eq("id", userId)
      .maybeSingle();
  ...
  const [showMemberships, movieMemberships, watched] = await Promise.all([
    atStage("show_memberships", () =>
      loadExportPages<ExportShowMembershipRow>((from, to) =>
        client
          .from("user_shows")
          .select("id,media_item_id,status,is_favourite,created_at")
          .eq("user_id", userId)
          .order("id", { ascending: true })
          .range(from, to),
      ),
    ),
    atStage("movie_memberships", () =>
      loadExportPages<ExportMovieMembershipRow>((from, to) =>
        client
          .from("user_movies")
          .select("id,media_item_id,watched_at,is_favourite,created_at")
          .eq("user_id", userId)
          .order("id", { ascending: true })
          .range(from, to),
      ),
    ),
    atStage("watched_history", () =>
      loadExportPages<ExportWatchedRow>((from, to) =>
        client
  ...
  const episodes = await atStage("episode_metadata", () =>
    loadEpisodeMetadata(
      client,
  ...
  const media = await atStage("media_metadata", () =>
    loadMediaMetadata(client, mediaIds),
  );
  return {
    profile,
    showMemberships,
