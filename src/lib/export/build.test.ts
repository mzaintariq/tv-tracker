...
function source(): ExportSourceData {
  return {
    ...
    movieMemberships: [
      {
        id: "movie-watch",
        media_item_id: "movie-60",
        watched_at: "2020-05-01T12:30:00+05:00",
        is_favourite: true,
        created_at: "2020-04-01T00:00:00Z",
      },
      {
        id: "movie-next",
        media_item_id: "movie-50",
        watched_at: null,
        is_favourite: false,
    ...
    watched: [
      {
        ...
        watched_at: "2022-01-02T03:04:05+05:00",
      },
      {
        id: "watched-special",
...
describe("TrackTV user export assembly", () => {
  it("builds the exact versioned portable shape and deterministic order", () => {
    ...
    expect(result.tvShows.map((show) => show.providerIds.tmdb)).toEqual([
      10, 20, 30, 40,
    ]);
    expect(result.movies.map((movie) => movie.providerIds.tmdb)).toEqual([
      50, 60,
    ]);
  });
  ...
  it("exports current states, favourites, empty shows, and only known removed tracking", () => {
    ...
    expect(removed.watchedEpisodes[0]).toMatchObject({
    ...
    expect(result.tvShows[1].tracking).toEqual({
      inLibrary: true,
      status: "active",
      isFavourite: true,
      addedAt: "2026-01-01T00:00:00Z",
    });
    expect(result.tvShows[2]).toMatchObject({
      tracking: { inLibrary: true, status: "dropped" },
      watchedEpisodes: [],
    });
    expect(result.tvShows[3]).toMatchObject({
      tracking: { inLibrary: true, status: "paused" },
      watchedEpisodes: [],
    });
  });

  it("preserves timestamp instants and sorts episodes by coordinates", () => {
    const episodes = buildTrackTvExport(source(), generatedAt).tvShows[1]
      .watchedEpisodes;
    expect(episodes.map((episode) => episode.episodeNumber)).toEqual([1, 2]);
    expect(episodes[1].watchedAt).toBe("2022-01-02T03:04:05+05:00");
    expect(Date.parse(episodes[1].watchedAt)).toBe(
      Date.parse("2022-01-01T22:04:05Z"),
    );
  });
  ...
  it("derives current movie states without reconstructing removed movies", () => {
    ...
    expect(movies[1]).toMatchObject({
      state: "watched",
      watchedAt: "2020-05-01T12:30:00+05:00",
      isFavourite: true,
      runtimeMinutes: 120,
    });
  ...
  it("exports an empty account", () => {
    const empty = source();
    empty.showMemberships = [];
    empty.movieMemberships = [];
    empty.watched = [];
    empty.episodes = [];
    empty.media = [];
    expect(buildTrackTvExport(empty, generatedAt)).toMatchObject({
      tvShows: [],
      movies: [],
    });
  });
  ...
  it("fails rather than silently dropping missing required metadata", () => {
    ...
    missing.episodes = missing.episodes.filter(
      (episode) => episode.id !== "episode-1",
    );
    expect(() => buildTrackTvExport(missing, generatedAt)).toThrow(
      ExportDataIntegrityError,
    );
  });
  ...
  it("does not expose internal, identity, import, or synchronization fields", () => {
    const serialized = JSON.stringify(
      buildTrackTvExport(source(), generatedAt),
    );
