...
export type ExportShowMembershipRow = {
  id: string;
...
export type ExportMovieMembershipRow = {
  id: string;
  media_item_id: string;
...
export type ExportWatchedRow = {
  id: string;
  episode_id: string;
  watched_at: string;
};
export type ExportEpisodeRow = {
  id: string;
  media_item_id: string;
...
export class ExportDataIntegrityError extends Error {
  constructor() {
    super("Required export metadata is missing or invalid.");
...
function compareEpisodes(
  left: TrackTvExportWatchedEpisode,
  right: TrackTvExportWatchedEpisode,
): number {
  const leftTmdb = left.providerIds.tmdb;
  const rightTmdb = right.providerIds.tmdb;
  const tmdbOrder =
    leftTmdb === null && rightTmdb === null
      ? 0
      : leftTmdb === null
        ? 1
        : rightTmdb === null
...
export function buildTrackTvExport(
  ...
  for (const watched of source.watched) {
    ...
    if (!parent || parent.media_type !== "tv")
      throw new ExportDataIntegrityError();
  ...
  const tvShows: TrackTvExportShow[] = source.showMemberships.map(
    (membership) => {
      ...
      return {
        providerIds: { tmdb: media.tmdb_id },
        title: media.title,
        firstAirDate: media.release_date,
        tracking: {
          inLibrary: true,
          status: membership.status,
          isFavourite: membership.is_favourite,
          addedAt: requireTimestamp(membership.created_at),
        },
        watchedEpisodes: [...(watchedByMediaId.get(media.id) ?? [])].sort(
          compareEpisodes,
        ),
      };
    },
  ...
  for (const [mediaId, watchedEpisodes] of watchedByMediaId) {
    ...
    tvShows.push({
      ...
      title: media.title,
      firstAirDate: media.release_date,
      tracking: { inLibrary: false },
      watchedEpisodes: [...watchedEpisodes].sort(compareEpisodes),
    });
  }

  const movies: TrackTvExportMovie[] = source.movieMemberships.map(
    (membership) => {
      const media = mediaById.get(membership.media_item_id);
      ...
      return {
        ...
        watchedAt:
          membership.watched_at === null
            ? null
            : requireTimestamp(membership.watched_at),
        isFavourite: membership.is_favourite,
      };
    },
  );
  ...
  tvShows.sort(
    (left, right) =>
      left.providerIds.tmdb - right.providerIds.tmdb ||
      compareText(left.title, right.title),
  );
  movies.sort(
    (left, right) =>
  ...
  return {
    schema: TRACKTV_EXPORT_SCHEMA,
    version: TRACKTV_EXPORT_VERSION,
    generatedAt,
    profile: {
