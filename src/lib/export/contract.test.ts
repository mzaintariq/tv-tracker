...
describe("user export implementation contract", () => {
  ...
  it("selects only explicit portable and loader identity fields", () => {
    const compactLoader = loader.replace(/\s+/g, "").replace(/,\)/g, ")");
    expect(compactLoader).toContain(
    ...
    expect(compactLoader).toContain('select("id,episode_id,watched_at")');
    expect(compactLoader).toContain(
