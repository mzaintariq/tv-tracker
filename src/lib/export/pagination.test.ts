...
describe("export paging and bounded metadata work", () => {
  it("pages at 500 rows beyond 1,000 and handles an exact-full final page", async () => {
    ...
    const rows = await loadExportPages(async (from, to) => {
      calls.push([from, to]);
  ...
  it("deduplicates IDs into 200-item chunks", () => {
    const ids = [
      ...Array.from({ length: 401 }, (_, index) => `id-${index}`),
      "id-0",
      "id-200",
    ];
    expect(uniqueChunks(ids).map((chunk) => chunk.length)).toEqual([
      200, 200, 1,
    ]);
  });
  ...
  it("rejects a failed page without returning partial data", async () => {
    await expect(
      loadExportPages(async (from) =>
        from === 0
          ? {
              data: Array.from({ length: 500 }, (_, id) => ({ id })),
              error: null,
            }
          : { data: null, error: new Error("private detail") },
      ),
    ).rejects.toThrow("Export data could not be loaded.");
  });
});
