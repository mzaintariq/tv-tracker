...
export type MovieSnapshot<TMedia extends MovieLibraryMedia = MediaItem> = {
  membership: UserMovie;
  media: TMedia;
};
export type MovieSections<
  TMedia extends MovieLibraryMedia = MovieLibraryMedia,
> = {
...
const titleCompare = <TMedia extends MovieLibraryMedia>(
  left: MovieSnapshot<TMedia>,
  right: MovieSnapshot<TMedia>,
) =>
...
const watchedCompare = <TMedia extends MovieLibraryMedia>(
  left: MovieSnapshot<TMedia>,
  right: MovieSnapshot<TMedia>,
) =>
  (right.membership.watched_at ?? "").localeCompare(
    left.membership.watched_at ?? "",
...
export function deriveMovieSections<TMedia extends MovieLibraryMedia>(
  ...
  const favourites = all
    .filter((movie) => movie.membership.is_favourite)
    .sort(titleCompare);
  return {
    movies: all,
    watchNext,
