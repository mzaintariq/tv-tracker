...
export function extractTrailingYearHint(sourceTitle: string): TitleHint {
  ...
  return match
    ? { searchTitle: match[1].trim(), year: Number(match[2]) }
    : { searchTitle: sourceTitle.trim(), year: null };
}
...
export function candidateDisplay(candidate: {
  id: number;
  title?: string;
  name?: string;
  ...
  return {
    ...
    year: candidateYear(candidate.release_date ?? candidate.first_air_date),
...
export function selectUniqueExactMovieCandidate<
  T extends {
    title: string;
    original_title?: string;
    release_date?: string | null;
...
>(
  ...
  const exact = candidates.filter(
    (item) =>
      ...
      normalize(item.original_title ?? "") === normalizedTitle,
  );
...
export function withoutConflictingYear<
  T extends { release_date?: string | null; first_air_date?: string | null },
>(candidates: T[], knownYear: number | null): T[] {
  if (!knownYear) return candidates;
  return candidates.filter((item) => {
    const year = candidateYear(item.release_date ?? item.first_air_date);
    return year === null || year === knownYear;
  });
}

export function shouldFallbackYearSearch(
  candidates: Array<{
    title?: string;
    name?: string;
    original_title?: string;
  ...
  return !candidates.some(
    (item) =>
      normalize(item.title ?? item.name ?? "") === normalizedTitle ||
      normalize(item.original_title ?? item.original_name ?? "") ===
        normalizedTitle,
  );
}
