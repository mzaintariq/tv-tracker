...
export type ImportIssueForDisplay = {
  id: string;
  ...
  selectedCandidate?: CandidateDisplay;
};
...
export function issueTypeLabel(type: string): string {
  ...
  if (type === "movie_favourites_confirmation")
    return "Movie favourites decision";
  return type
    .split("_")
...
function completedKey(issue: ImportIssueForDisplay): {
  key: string;
  label: string;
  expandable: boolean;
} {
  ...
  if (
    ["ambiguous_media", "unmatched_media"].includes(issue.issue_type) &&
    issue.mediaType
  )
    return {
      key: `media:${issue.status}:${issue.mediaType}`,
      label: `${issue.status === "resolved" ? "Resolved" : "Skipped"} ${mediaLabel}`,
      expandable: true,
    };
  if (issue.issue_type === "missing_episode_coordinate")
    return issue.status === "resolved"
      ? {
          key: "coordinates:resolved",
          label: "Previously unavailable episode records",
          expandable: true,
        }
      : {
          key: "coordinates:skipped",
          label: "Episode records not imported",
          expandable: true,
        };
  return {
    key: `decision:${issue.issue_type}:${issue.status}`,
    label: `${issueTypeLabel(issue.issue_type)} — ${issueStatusLabel(issue.status)}`,
    expandable: false,
  };
}
...
export function buildImportIssueDisplayModel(issues: ImportIssueForDisplay[]) {
  ...
  for (const issue of issues.filter((item) => item.status !== "open")) {
    const descriptor = completedKey(issue);
  ...
  return {
    total: issues.length,
    openBlocking: open.filter((issue) => issue.is_blocking).length,
    openNonBlocking: notices.length,
    resolved: issues.filter(
      (issue) => issue.status !== "open" && issue.status !== "skipped",
    ).length,
    skipped: issues.filter((issue) => issue.status === "skipped").length,
    needsAttention,
    notices,
    completedGroups: [...groups.values()].sort((a, b) =>
