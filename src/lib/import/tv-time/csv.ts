...
export function parseCsv(
  ...
  for (let index = 0; index < input.length; index += 1) {
    ...
    if (field.length > MAX_FIELD_LENGTH)
      throw new TvTimeImportError(
        "csv_field_too_large",
        "A CSV field exceeds the supported size.",
      );
  }
  if (quoted)
    throw new TvTimeImportError(
      "csv_unclosed_quote",
      "A CSV contains an unclosed quoted field.",
    );
  if (field || row.length) {
    row.push(field.replace(/\r$/, ""));
    records.push(row);
  ...
  if (
    ...
    throw new TvTimeImportError(
      "csv_schema_mismatch",
      "A TV Time CSV has an unsupported schema.",
    );
  }
  return values
    ...
    .map((value) => {
      ...
      return Object.fromEntries(
        expectedHeaders.map((header) => [header, supplied[header] ?? ""]),
      );
    });
}
