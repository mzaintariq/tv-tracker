...
function storedZip(entries: { name: string; value: string }[]): Uint8Array {
  ...
  const result = new Uint8Array(
    all.reduce((sum, value) => sum + value.length, 0),
  );
  let cursor = 0;
...
].map((name) => ({ name, value: "header\n" }));

describe("TV Time ZIP security", () => {
  it("returns only allowlisted files and never opens sensitive entries", () => {
    const result = readAllowedTvTimeZip(
      storedZip([
        ...required,
        { name: "access_token.csv", value: "secret-token" },
      ]),
    );
  ...
  it.each(required)(
    "accepts a valid export when only the %s dataset is present",
    ({ name, value }) => {
      expect(readAllowedTvTimeZip(storedZip([{ name, value }]))).toEqual({
        [name]: value,
      });
    },
  );
  it("rejects an archive with no identifying TV Time dataset", () => {
    expect(() =>
      readAllowedTvTimeZip(
        storedZip([{ name: "stats-prod-cache.csv", value: "stats\n" }]),
      ),
  ...
  it("rejects traversal paths", () =>
    expect(() =>
      readAllowedTvTimeZip(
        storedZip([{ name: "../tracking-prod-records.csv", value: "x" }]),
      ),
    ).toThrow("path traversal"));
  it("rejects duplicate case-insensitive names", () =>
    expect(() =>
      readAllowedTvTimeZip(
        storedZip([...required, { name: "FOLLOWED_TV_SHOW.CSV", value: "x" }]),
      ),
    ).toThrow("duplicate filenames"));
  it("rejects nested archive extensions case-insensitively", () =>
    expect(() =>
      readAllowedTvTimeZip(storedZip([{ name: "payload.ZIP", value: "x" }])),
    ).toThrow("Nested paths and archives"));
  it("rejects a central filename that differs from its local filename", () => {
    const archive = storedZip(required);
    archive[30] ^= 1;
    expect(() => readAllowedTvTimeZip(archive)).toThrow(
      "does not match its local header",
    );
  });
