...
function validateName(name: string): void {
  if (
    ...
    fail("zip_unsafe_path", "The ZIP contains an unsafe path.");
  if (name.split("/").some((part) => part === ".." || part === "."))
    fail("zip_unsafe_path", "The ZIP contains path traversal.");
  if (name.includes("/") || name.toLocaleLowerCase("en-US").endsWith(".zip"))
    fail("zip_nested_path", "Nested paths and archives are not supported.");
}
...
function centralEntries(bytes: Uint8Array): {
  ...
  if (
    ...
    fail("zip_limits", "The ZIP entry count or directory is invalid.");
  ...
  for (let index = 0; index < count; index += 1) {
    if (
      ...
      fail("zip_invalid_directory", "The ZIP directory is malformed.");
    ...
    if (nextOffset > eocd || nextOffset > bytes.length)
      fail("zip_invalid_directory", "The ZIP directory is malformed.");
    ...
    if (names.has(folded))
      fail("zip_duplicate_name", "The ZIP contains duplicate filenames.");
    names.add(folded);
    if ((flags & 1) !== 0)
      fail("zip_encrypted", "Encrypted ZIP entries are not supported.");
    const externalAttributes = view.getUint32(offset + 38, true);
    const unixType = (externalAttributes >>> 16) & 0xf000;
    if (unixType === 0xa000)
      fail("zip_symlink", "ZIP symlinks are not supported.");
    entries.push({
      name,
      nameBytes,
      flags,
  ...
  if (offset !== directoryOffset + directorySize)
    fail(
      ...
      "The ZIP directory size does not match its entries.",
    );
...
function bindLocalHeaders(
  ...
  for (const entry of entries) {
    ...
    if (
      ...
      fail("zip_invalid_local", "A ZIP local header is malformed.");
    ...
    if (
      ...
      fail(
        "zip_header_mismatch",
        "A ZIP central entry does not match its local header.",
      );
    ...
    if (
      ...
      fail(
        "zip_header_mismatch",
        "A ZIP central entry does not match its local integrity metadata.",
      );
  ...
  for (let index = 1; index < ranges.length; index += 1) {
    if (ranges[index].localOffset < (ranges[index - 1].dataEnd ?? 0))
      fail("zip_overlap", "ZIP local records or entry data ranges overlap.");
  }
}

export function readAllowedTvTimeZip(
  bytes: Uint8Array,
): Partial<Record<AllowedTvTimeFile, string>> {
  if (bytes.length > MAX_COMPRESSED_UPLOAD_BYTES)
    fail(
      "zip_compressed_too_large",
      "The ZIP exceeds the compressed upload limit.",
    );
  ...
  for (const entry of entries) {
    ...
    if (entry.uncompressedSize > MAX_ENTRY_BYTES)
      fail("zip_entry_too_large", "An import CSV exceeds the supported size.");
    if (
      entry.compressedSize === 0
        ? entry.uncompressedSize > 0
        : entry.uncompressedSize / entry.compressedSize > MAX_COMPRESSION_RATIO
    )
      fail("zip_ratio", "A ZIP entry has an unsafe compression ratio.");
    total += entry.uncompressedSize;
    if (total > MAX_ALLOWED_TOTAL_BYTES)
      fail(
        "zip_total_too_large",
        "The import CSVs exceed the supported total size.",
      );
    ...
    if (compressed.length !== entry.compressedSize)
      fail("zip_truncated", "A ZIP entry is truncated.");
    ...
    if (entry.method === 0) content = compressed;
      ...
      fail("zip_method", "A ZIP entry uses an unsupported compression method.");
    if (
      content.length !== entry.uncompressedSize ||
      crc32(content) !== entry.crc
    )
      fail("zip_integrity", "A ZIP entry failed its integrity check.");
    const decoded = new TextDecoder("utf-8", { fatal: true })
      .decode(content)
      .replace(/^\uFEFF/, "");
    result[entry.name as AllowedTvTimeFile] = decoded;
  }
  const identifyingFiles: AllowedTvTimeFile[] = [
    "tracking-prod-records.csv",
    "tracking-prod-records-v2.csv",
    "user_tv_show_data.csv",
    "followed_tv_show.csv",
  ];
  if (identifyingFiles.every((name) => result[name] === undefined))
    fail(
      "zip_missing_required",
      "The ZIP is missing required TV Time CSV files.",
    );
  return result;
}
