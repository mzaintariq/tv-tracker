...
describe("Phase 8 migration security contract", () => {
  it("contains owner-aware foreign keys", () => {
    ...
    expect(sql).toContain(
      "foreign key(mapping_id,user_id) references public.source_media_mappings(id,user_id)",
    );
    expect(sql).toContain(
      "foreign key(import_item_id,user_id) references public.import_items(id,user_id)",
    );
  });
  it("makes trusted functions service-role-only", () => {
    expect(sql).toContain(
      "revoke execute on function public.apply_tv_time_show_import",
    );
    expect(sql).toContain(
      "grant execute on function public.apply_tv_time_show_import",
    );
    expect(sql).toContain("to service_role");
    expect(sql).not.toContain("grant all on public.imports");
    expect(sql).toContain(
      "grant select,insert,update,delete on public.imports",
    );
  });
  it("implements leases and nullable cleanup", () => {
    ...
    expect(sql).toContain(
      "ii.match_status='matching' and ii.match_claim_expires_at<=now()",
    );
  ...
  it("preserves additive TrackTV state and all-or-nothing TV application", () => {
    expect(sql).toContain(
      "public.user_shows.is_favourite or excluded.is_favourite",
    );
    expect(sql).toContain(
      "coalesce(public.user_movies.watched_at,excluded.watched_at)",
    );
    expect(sql).toContain("application_status='blocked'");
    expect(sql.indexOf("if v_missing>0")).toBeLessThan(
      sql.indexOf("insert into public.watched_episodes"),
    );
  });
  it("initializes Analyze atomically under a per-fingerprint lock", () => {
    ...
    expect(sql).toContain(
      "if exists(select 1 from public.imports where id=v_import_id and status<>'parsing') then return v_import_id",
    );
  });
  ...
  it("serializes lifecycle mutations on the import row", () => {
    ...
    expect(
      sql.match(/where id=p_import_id and user_id=p_user_id for update/g)
        ?.length,
