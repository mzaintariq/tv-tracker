...
describe("import resolution sections",()=>{
  ...
  it("labels filters and contextual manual controls",()=>{const html=renderToStaticMarkup(createElement(ImportResolutionSections,{importId:"import",items:[item("1","tv","Alpha")]}));expect(html).toContain('role="group"');expect(html).toContain('aria-label="Filter unresolved tv shows"');expect(html).toContain('for="manual-tmdb-1"');expect(html).toContain("Manual TMDB ID for Alpha");expect(html).toContain('aria-label="Confirm TMDB ID for Alpha"');});
