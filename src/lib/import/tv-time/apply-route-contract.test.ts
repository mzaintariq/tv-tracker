...
describe("Apply route preflight contract", () => {
  it("rejects a wrong fingerprint before entering Apply", () => {
    const source = readFileSync(
      "src/app/api/imports/[importId]/apply/route.ts",
      "utf8",
    );
    expect(source.indexOf("fingerprint_mismatch")).toBeLessThan(
      source.indexOf("const result = await applyNormalizedImport"),
    );
  });
  it("logs one sanitized structured Apply diagnostic without raw source fields", () => {
    ...
    const log = source.slice(
      source.indexOf('event: "tv_time_apply_paused"'),
      source.indexOf(
