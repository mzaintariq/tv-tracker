...
vi.mock("@/lib/tmdb/mappers", () => ({
  mapTmdbMovieDetailsToCacheRow: vi.fn(),
}));
...
describe("completed TV Time import replay", () => {
  ...
  it("returns an idempotent no-op without final-state writes", async () => {
    ...
    await expect(
      applyNormalizedImport("user-id", "import-id", normalized),
    ).resolves.toEqual({
      applied: 0,
      blocked: 0,
...
describe("paused TV Time Apply resume", () => {
  ...
  it("starts paused Apply after digest preflight and preserves final-state items", async () => {
    const imports = {
      ...
      single: vi
        .fn()
        .mockResolvedValue({ data: { status: "paused" }, error: null }),
    };
    imports.select.mockReturnValue(imports);
    imports.eq.mockReturnValue(imports);
    const finalItems = [
      {
        id: "applied",
        mapping_id: null,
      ...
      },
      {
        ...
        media_type: "movie",
        source_key: "b",
        import_mode: "watch_next_movie",
        match_status: "skipped",
        application_status: "skipped",
        source_item_digest: "y",
      },
    ];
    const items = {
      select: vi.fn(),
      eq: vi.fn(),
      order: vi.fn(),
      range: vi.fn().mockResolvedValue({ data: finalItems, error: null }),
    };
    items.select.mockReturnValue(items);
    items.eq.mockReturnValue(items);
    items.order.mockReturnValue(items);
    const mappings = {
      select: vi.fn(),
      eq: vi.fn(),
      order: vi.fn(),
    ...
    mappings.select.mockReturnValue(mappings);
    mappings.eq.mockReturnValue(mappings);
    mappings.order.mockReturnValue(mappings);
    const issues = {
      select: vi.fn(),
      eq: vi.fn(),
      maybeSingle: vi.fn().mockResolvedValue({ data: null, error: null }),
    };
    issues.select.mockReturnValue(issues);
    issues.eq.mockReturnValue(issues);
    const admin = {
    ...
    const normalized = {
      ...
      summary: {},
    } as unknown as NormalizedTvTimeImport;
    await expect(
      applyNormalizedImport("user", "import", normalized),
    ).resolves.toEqual({
      applied: 0,
      blocked: 0,
      tvUnits: { applied: 0, blocked: 0 },
    ...
    expect(admin.rpc).toHaveBeenCalledWith("start_tv_time_import_apply", {
      p_user_id: "user",
      p_import_id: "import",
    });
    expect(synchronizeShow).not.toHaveBeenCalled();
  ...
  it("does not transition a paused import when item digests fail", async () => {
    ...
    await expect(
      applyNormalizedImport("user", "import", {
        shows: [],
        movies: [],
      } as unknown as NormalizedTvTimeImport),
...
describe("Apply durable accounting", () => {
  it("loads every page beyond the PostgREST 1,000-row ceiling", async () => {
    ...
    const fetched = await loadAllApplyPages(async (from, to) => ({
  ...
  it("assigns a safe persisted reason to every eligibility failure", () => {
    expect(
      applyEligibilityFailure(
        { match_status: "pending", mapping_id: null },
        undefined,
      ),
    ).toBe("apply_match_not_confirmed");
    expect(
      applyEligibilityFailure(
        { match_status: "confirmed", mapping_id: "missing" },
        undefined,
      ),
    ).toBe("apply_mapping_missing");
    expect(
      applyEligibilityFailure(
        { match_status: "confirmed", mapping_id: "mapping" },
        { id: "mapping", tmdb_id: null, resolution_status: "auto_confirmed" },
      ),
    ).toBe("apply_tmdb_id_missing");
    expect(
      applyEligibilityFailure(
        { match_status: "confirmed", mapping_id: "mapping" },
        { id: "mapping", tmdb_id: 1, resolution_status: "ambiguous" },
      ),
    ).toBe("apply_mapping_not_confirmed");
  });
});
