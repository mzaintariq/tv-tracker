...
import {
  automaticMatchingProgress,
  matchingProgressFromCounts,
...
const items = (...statuses: string[]) =>
  statuses.map((match_status) => ({ match_status }));
type SnapshotCounts = {
  status: string;
  total: number;
  unfinished: number;
...
const snapshot = (overrides: Partial<SnapshotCounts> = {}) =>
  matchingProgressFromCounts({
    status: "matching",
    total: 10,
    unfinished: 6,
...
describe("automatic matching progress", () => {
  it("starts above zero for durable confirmed mappings", () => {
    expect(
      automaticMatchingProgress(items("confirmed", "pending", "pending")),
    ).toMatchObject({ processed: 1, percentage: 33 });
  });

  it("counts pending and matching leases as unfinished", () => {
    expect(
      automaticMatchingProgress(items("pending", "matching", "confirmed")),
    ).toMatchObject({ remaining: 2, processed: 1 });
  });

  it("counts ambiguous and unmatched results as processed", () => {
    expect(
      automaticMatchingProgress(items("ambiguous", "unmatched", "pending")),
    ).toMatchObject({ processed: 2, needsReview: 2, percentage: 66 });
  });

  it("reaches 100 percent when automatic work is exhausted despite manual review", () => {
    expect(
      automaticMatchingProgress(
        items("confirmed", "ambiguous", "unmatched", "failed", "skipped"),
      ),
    ).toMatchObject({
      processed: 5,
      remaining: 0,
  ...
  it("reconstructs identical progress from the same persisted statuses", () => {
    ...
    expect(automaticMatchingProgress([...persisted])).toEqual(
      automaticMatchingProgress([...persisted]),
    );
  });
  ...
  it("renders a zero-item import safely", () => {
    ...
    expect(
      renderToStaticMarkup(
        createElement(ImportMatchingProgress, {
          importId: "import",
          initialProgress: progress,
        }),
      ),
    ).toContain("0 of 0 checked");
  });
  ...
  it("renders accessible persisted counts and progress values", () => {
    const markup = renderToStaticMarkup(
      createElement(ImportMatchingProgress, {
        importId: "import",
        initialProgress: automaticMatchingProgress(
          items("confirmed", "ambiguous", "pending", "matching"),
        ),
      }),
    );
  ...
  it("updates stale counts serially without overlapping requests", async () => {
    ...
    const fetchProgress = vi.fn(
      () =>
        new Promise<MatchingProgressSnapshot>((resolve) => {
    ...
    resolveFirst?.(snapshot({ unfinished: 5 }));
  ...
  it("stops polling when no pending or matching work remains", async () => {
    ...
    startMatchingProgressPolling({
      fetchProgress: async () =>
        snapshot({ status: "awaiting_resolution", unfinished: 0 }),
      onProgress,
      shouldContinue: (next) =>
        next.status === "matching" && next.remaining > 0,
      schedule,
    });
    await Promise.resolve();
    await Promise.resolve();
  ...
  it("continues with review items while automatic work remains and retries polling failures", async () => {
    ...
    const fetchProgress = vi
      .fn<() => Promise<MatchingProgressSnapshot>>()
      .mockRejectedValueOnce(new Error("temporary"))
      .mockResolvedValue(snapshot({ unfinished: 4, needsReview: 5 }));
    const poller = startMatchingProgressPolling({
      fetchProgress,
      onProgress: vi.fn(),
      onError: vi.fn(),
      shouldContinue: (next) => next.remaining > 0,
      schedule: (callback) => {
  ...
  it("cancels scheduled polling and ignores stale decreasing responses", async () => {
    const scheduled: Array<() => void> = [];
    const fetchProgress = vi.fn(async () => snapshot());
    const poller = startMatchingProgressPolling({
      fetchProgress,
      onProgress: vi.fn(),
      shouldContinue: () => true,
    ...
    expect(
      reconcileMatchingProgress(
        snapshot({ unfinished: 3 }),
        snapshot({ unfinished: 5 }),
      ),
    ).toEqual(snapshot({ unfinished: 3 }));
  });
});
