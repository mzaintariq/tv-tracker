...
export async function loadMatchingProgress(
  client: SupabaseClient<Database>,
  importId: string,
  userId: string,
): Promise<MatchingProgressSnapshot | null> {
  const base = () =>
    client
      .from("import_items")
      ...
      .eq("user_id", userId);
  const [session, total, unfinished, confirmed, review, skipped] =
    await Promise.all([
      client
        .from("imports")
        .select("status")
  ...
  return matchingProgressFromCounts({
