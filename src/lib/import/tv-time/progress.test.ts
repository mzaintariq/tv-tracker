...
function sampleProgress(
  overrides: Partial<ImportApplyProgress> = {},
): ImportApplyProgress {
  return {
    status: "applying",
    applyPhase: "applying",
...
describe("Apply progress polling", () => {
  it("starts while applying and never overlaps requests", async () => {
    ...
    const fetchProgress = vi.fn(
      () =>
        new Promise<ImportApplyProgress>((resolve) => {
          finishFirst = resolve;
        }),
    );
  ...
  it.each(["completed", "paused"])(
    "performs one final persisted fetch and stops on %s",
    async (status) => {
      const scheduled: Array<() => void> = [];
      ...
      startImportProgressPolling({
        fetchProgress: async () => sampleProgress({ status }),
        onProgress,
        shouldContinue: () => true,
        schedule,
      });
...
describe("Apply progress presentation", () => {
  ...
  it("restores counts, separates blocked/skipped, and shows a safe paused error", () => {
    const markup = renderToStaticMarkup(
      createElement(ImportApplyProgressPanel, {
        importId: "00000000-0000-0000-0000-000000000001",
        initialProgress: sampleProgress({
          status: "paused",
          applyPhase: "paused",
          lastErrorCode: "tv_metadata_episode_identity_conflict",
        }),
        requestPending: false,
      }),
    );
    expect(markup).toContain(
      "4 units applied · 1 units blocked · 5 units remaining",
    );
  ...
  it("replaces stale paused state immediately and rejects older polling state", () => {
    ...
    expect(
      reconcileApplyProgress(
        sampleProgress({ tvApplied: 289 }),
        sampleProgress({ tvApplied: 288 }),
      ),
    ).toMatchObject({ tvApplied: 289 });
  });
  ...
  it("does not render a second generic lifecycle message from the upload response", () => {
    ...
    expect(source).toContain(
      'mode === "apply" && result?.code === "apply_failed"',
    );
  });
  ...
  it("exposes only aggregate fields from the authenticated progress function", () => {
    ...
    const sql = migration.slice(
      migration.indexOf(
        "create function public.get_tv_time_import_apply_progress",
      ),
      migration.indexOf(
        "revoke execute on function public.get_tv_time_import_apply_progress",
      ),
    );
    const route = readFileSync(
      "src/app/api/imports/[importId]/progress/route.ts",
      "utf8",
    );
    for (const field of [
      "tvTotalEligible",
