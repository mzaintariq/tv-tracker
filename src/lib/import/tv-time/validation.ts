...
export function isMatchContext(value: unknown): value is MatchContext {
  ...
  if (object.kind === "movie") {
    return (
      exactKeys(object, ["version", "kind", "releaseDate"]) &&
      (object.releaseDate === null ||
        (typeof object.releaseDate === "string" &&
          /^\d{4}-\d{2}-\d{2}$/.test(object.releaseDate)))
    );
  }
  if (
    object.kind !== "tv" ||
    !exactKeys(object, ["version", "kind", "coordinates"]) ||
    !Array.isArray(object.coordinates) ||
    object.coordinates.length > MAX_TV_COORDINATES
  )
    return false;
  ...
  for (const coordinate of object.coordinates) {
    ...
    if (
      !exactKeys(item, ["seasonNumber", "episodeNumber"]) ||
      !Number.isInteger(item.seasonNumber) ||
      !Number.isInteger(item.episodeNumber)
    )
      return false;
    const season = item.seasonNumber as number;
    const episode = item.episodeNumber as number;
    if (season < 0 || season > 10_000 || episode < 1 || episode > 100_000)
      return false;
