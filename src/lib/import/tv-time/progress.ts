...
export function applyingProgress(
  progress: ImportApplyProgress,
...
export function reconcileApplyProgress(
  current: ImportApplyProgress,
  incoming: ImportApplyProgress,
): ImportApplyProgress {
  if (
    Date.parse(incoming.updatedAt) < Date.parse(current.updatedAt) ||
    incoming.tvApplied < current.tvApplied ||
    incoming.movieApplied < current.movieApplied
  )
    return current;
  return incoming;
}
...
export function pausedApplyMessage(code: string | null): string {
  if (code === "tv_metadata_episode_identity_conflict")
    return "TV episode metadata could not be reconciled safely.";
  if (code === "apply_resume_transition_failed")
    return "The import could not resume safely.";
  return code
    ? `The import paused safely (${code}).`
    : "The import paused safely.";
}
...
export function startImportProgressPolling(options: PollOptions): () => void {
  ...
  const tick = async () => {
    ...
    try {
      ...
      if (isApplyTerminal(progress.status)) {
        if (finalFetch) stopped = true;
        else finalFetch = true;
    ...
    if (!stopped)
      timer = schedule(() => void tick(), options.intervalMs ?? 1500);
  };
