  ...
  it("collapses coordinates to the latest event and gives watched movies priority", () => {
    const files = {
      ...
      "user_tv_show_data.csv": csv("user_tv_show_data.csv", [
        { tv_show_id: "10", tv_show_name: "Example", is_favorited: "1" },
      ]),
      "followed_tv_show.csv": csv("followed_tv_show.csv", [
        {
          tv_show_id: "10",
          tv_show_name: "Example",
          active: "1",
          archived: "0",
        },
      ]),
    ...
    expect(result.shows[0].episodeEvents[0]).toMatchObject({
      sourceEventCount: 2,
      watchedAt: { instant: "2025-01-01T00:00:00.000Z" },
    });
    expect(result.shows[0]).toMatchObject({
      importMode: "active_membership",
      isFavourite: true,
    });
  ...
  it("uses current show sources when followed_tv_show.csv is absent", () => {
    const result = normalizeTvTimeExport({
      "tracking-prod-records-v2.csv": csv("tracking-prod-records-v2.csv", [
        ...
        {
          key: "user-series-archived",
          s_id: "20",
          is_followed: "true",
          is_archived: "true",
          series_name: "Archived",
        },
        {
          key: "watch-episode-archived",
          s_id: "20",
          s_no: "1",
          ep_id: "200",
          ep_no: "1",
    ...
    expect(
      result.shows.map((show) => [show.tvTimeShowId, show.importMode]),
    ).toEqual([
...
describe("canonical source identity", () => {
  it("normalizes scientific integer IDs without Number precision loss", () => {
    expect(normalizeNumericSourceId("9.007199254740993e15")).toBe(
      "9007199254740993",
    );
  ...
  it("sorts object keys recursively for stable item digests", () => {
    ...
    expect(itemDigest({ a: 1, b: { c: 2 } })).toBe(
      itemDigest({ b: { c: 2 }, a: 1 }),
    );
  });
});
...
describe("empty source titles", () => {
  it.each(["", "   "])("uses the existing TV fallback for %j", (title) => {
    const result = normalizeTvTimeExport({
      ...
      "tracking-prod-records.csv": csv("tracking-prod-records.csv", []),
      "user_tv_show_data.csv": csv("user_tv_show_data.csv", [
        { tv_show_id: "10", tv_show_name: title },
      ]),
      "followed_tv_show.csv": csv("followed_tv_show.csv", [
        { tv_show_id: "10", tv_show_name: title, active: "1", archived: "0" },
      ]),
    });
