...
async function renderPoster(
  props: Partial<React.ComponentProps<typeof MediaPoster>> = {},
): Promise<ReactTestRenderer> {
  let renderer: ReactTestRenderer | undefined;
  await act(() => {
    renderer = create(
      createElement(MediaPoster, {
        source: "/poster.jpg",
        title: "The Example",
...
describe("MediaPoster", () => {
  it("resolves TMDB paths and complete HTTPS URLs", () => {
    expect(resolvePosterSource("/poster.jpg", "w500")).toBe(
      "https://image.tmdb.org/t/p/w500/poster.jpg",
    );
    expect(resolvePosterSource("https://cdn.example.com/poster.jpg")).toBe(
  ...
  it.each([
    ...
    "javascript:alert(1)",
  ])("renders a fallback for missing or unsafe source %s", async (source) => {
  ...
  it("preserves caller classes, sizes, and loader behavior", async () => {
    const tmdb = await renderPoster({
      className: "custom-image",
      sizes: "50vw",
    });
    expect(image(tmdb).props).toMatchObject({
      className: "custom-image",
      sizes: "50vw",
      fill: true,
      unoptimized: false,
    });
    const external = await renderPoster({
  ...
  it("tries a genuinely changed source after a previous failure", async () => {
    ...
    expect(image(renderer).props.src).toBe(
      "https://image.tmdb.org/t/p/w342/replacement.jpg",
    );
  });
});
