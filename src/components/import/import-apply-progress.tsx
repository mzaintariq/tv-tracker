...
function ProgressBar({ value }: { value: number }) {
  return <div className="h-2 overflow-hidden rounded-full bg-[var(--border)]" aria-hidden="true"><div className="h-full bg-[var(--accent)] transition-[width]" style={{ width: `${value}%` }} /></div>;
}
...
export function ImportApplyProgressPanel({ importId, initialProgress, requestPending }: { importId: string; initialProgress: ImportApplyProgress; requestPending: boolean }) {
  ...
  return <div className="space-y-4 rounded-lg border p-4" aria-live="polite">
    ...
    <div className="space-y-2"><div className="flex justify-between gap-3"><h3 className="font-medium">TV transactional units</h3><span>{tvPercent}%</span></div><ProgressBar value={tvPercent} /><p className="text-sm text-[var(--muted)]">{displayed.tvApplied} units applied · {displayed.tvBlocked} units blocked · {displayed.tvRemaining} units remaining</p></div>
    <div className="space-y-2"><div className="flex justify-between gap-3"><h3 className="font-medium">Movie items</h3><span>{moviePercent}%</span></div><ProgressBar value={moviePercent} /><p className="text-sm text-[var(--muted)]">{displayed.movieApplied} items applied · {displayed.movieRemaining} items remaining</p></div>
