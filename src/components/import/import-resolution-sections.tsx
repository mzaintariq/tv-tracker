...
function ResolutionSection({ importId, mediaType, items }: { importId: string; mediaType: "tv" | "movie"; items: ResolutionItem[] }) {
  ...
  return <section className="space-y-3"><div><h2 className="text-xl font-semibold">{label} needing resolution ({items.length})</h2><div className="mt-3 flex flex-wrap gap-2"><button className="rounded-lg border px-3 py-2" aria-pressed={filter === "all"} onClick={() => setFilter("all")}>All</button><button className="rounded-lg border px-3 py-2" aria-pressed={filter === "ambiguous"} onClick={() => setFilter("ambiguous")}>Several possible matches</button><button className="rounded-lg border px-3 py-2" aria-pressed={filter === "unmatched"} onClick={() => setFilter("unmatched")}>No match found</button></div>{items.length > 10 ? <input className="mt-3 w-full max-w-sm rounded-lg border px-3 py-2" aria-label={`Search unresolved ${label.toLocaleLowerCase()}`} placeholder="Search by title" value={search} onChange={(event) => setSearch(event.target.value)} /> : null}</div><button disabled={pending || items.length === 0} className="rounded-lg border px-3 py-2 text-[var(--danger)]" onClick={() => void skipAll()}>{pending ? "Skipping…" : `Skip all unresolved ${label} (${items.length})`}</button>{message ? <p role={message.includes("could not") ? "alert" : "status"}>{message}</p> : null}{visible.map((item) => <article key={item.id} className="space-y-2 rounded-xl border p-4"><p className="font-medium">{item.sourceTitle} <span className="text-[var(--muted)]">({item.mediaType === "tv" ? "TV show" : "Movie"})</span></p><p className="text-sm text-[var(--muted)]">{item.matchStatus === "ambiguous" ? "Several possible matches" : "No match found"}</p><ResolutionControls importId={importId} itemId={item.id} candidates={item.candidates} candidateMetadata={item.candidateMetadata} /></article>)}</section>;
}
