...
function ResolutionSection({ importId, mediaType, items }: { importId: string; mediaType: "tv" | "movie"; items: ResolutionItem[] }) {
  ...
  return <section className="min-w-0 space-y-3"><div className="min-w-0"><h2 className="break-words text-xl font-semibold">{label} needing resolution ({items.length})</h2><div className="mt-3 flex flex-wrap gap-2" role="group" aria-label={`Filter unresolved ${label.toLocaleLowerCase()}`}><button className={filter === "all" ? "touch-target max-w-full whitespace-normal rounded-lg bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-[var(--accent-foreground)] shadow-[inset_0_0_0_2px_var(--foreground)]" : "interactive-control touch-target max-w-full whitespace-normal rounded-lg border bg-[var(--surface)] px-3 py-2 text-sm font-semibold text-[var(--foreground)]"} aria-pressed={filter === "all"} onClick={() => setFilter("all")}>All</button><button className={filter === "ambiguous" ? "touch-target max-w-full whitespace-normal rounded-lg bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-[var(--accent-foreground)] shadow-[inset_0_0_0_2px_var(--foreground)]" : "interactive-control touch-target max-w-full whitespace-normal rounded-lg border bg-[var(--surface)] px-3 py-2 text-sm font-semibold text-[var(--foreground)]"} aria-pressed={filter === "ambiguous"} onClick={() => setFilter("ambiguous")}>Several possible matches</button><button className={filter === "unmatched" ? "touch-target max-w-full whitespace-normal rounded-lg bg-[var(--accent)] px-3 py-2 text-sm font-semibold text-[var(--accent-foreground)] shadow-[inset_0_0_0_2px_var(--foreground)]" : "interactive-control touch-target max-w-full whitespace-normal rounded-lg border bg-[var(--surface)] px-3 py-2 text-sm font-semibold text-[var(--foreground)]"} aria-pressed={filter === "unmatched"} onClick={() => setFilter("unmatched")}>No match found</button></div>{items.length > 10 ? <div className="mt-3 min-w-0"><label htmlFor={searchId} className="block break-words text-sm font-medium">Search unresolved {label.toLocaleLowerCase()}</label><input id={searchId} className="interactive-control touch-target mt-2 w-full min-w-0 max-w-sm rounded-lg border bg-[var(--surface)] px-3 py-2 text-[var(--foreground)]" placeholder="Search by title" value={search} onChange={(event) => setSearch(event.target.value)} /></div> : null}</div><button disabled={pending || items.length === 0} className="interactive-control touch-target max-w-full whitespace-normal rounded-lg border bg-[var(--surface)] px-3 py-2 text-sm font-semibold text-[var(--danger)]" onClick={() => void skipAll()}>{pending ? "Skipping…" : `Skip all unresolved ${label} (${items.length})`}</button>{message ? <p className="break-words" role={message.includes("could not") ? "alert" : "status"}>{message}</p> : null}{visible.map((item) => <article key={item.id} className="min-w-0 space-y-2 rounded-xl border p-4"><p className="break-words font-medium">{item.sourceTitle} <span className="text-[var(--muted)]">({item.mediaType === "tv" ? "TV show" : "Movie"})</span></p><p className="break-words text-sm text-[var(--muted)]">{item.matchStatus === "ambiguous" ? "Several possible matches" : "No match found"}</p><ResolutionControls importId={importId} itemId={item.id} itemLabel={item.sourceTitle} candidates={item.candidates} candidateMetadata={item.candidateMetadata} /></article>)}</section>;
}
