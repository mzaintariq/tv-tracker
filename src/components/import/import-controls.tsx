...
export function ResolutionControls({ importId, itemId, itemLabel = "this import item", candidates, candidateMetadata }: { importId: string; itemId: string; itemLabel?: string; candidates: number[]; candidateMetadata?: CandidateDisplay[] }) {
  ...
  return <div className="min-w-0 space-y-3"><div className="grid min-w-0 gap-3 sm:grid-cols-2">{candidatesForDisplay(candidates, candidateMetadata).map((candidate) => <CandidateCard key={candidate.id} candidate={candidate} disabled={pending} onUse={() => void run(() => confirmImportCandidate(importId, itemId, candidate.id))} />)}</div><div className="min-w-0 space-y-2"><label htmlFor={manualIdInput} className="block break-words text-sm font-medium">Manual TMDB ID for {itemLabel}</label><p id={manualHelp} className="break-words text-sm text-[var(--muted)]">Enter the numeric TMDB identifier when none of the suggested matches is correct.</p><div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:flex-wrap"><input id={manualIdInput} className="w-full min-w-0 max-w-full rounded-lg border px-3 py-2 sm:w-36" inputMode="numeric" aria-describedby={manualHelp} placeholder="TMDB ID" value={manualId} onChange={(event) => setManualId(event.target.value)} /><button aria-label={`Confirm TMDB ID for ${itemLabel}`} disabled={pending || !/^\d+$/.test(manualId)} className="max-w-full whitespace-normal rounded-lg border px-3 py-2" onClick={() => void run(() => confirmImportCandidate(importId, itemId, Number(manualId)))}>Confirm ID</button><button aria-label={`Skip ${itemLabel} from this import`} disabled={pending} className="max-w-full whitespace-normal rounded-lg border px-3 py-2 text-[var(--danger)]" onClick={() => void run(() => skipImportItem(importId, itemId))}>Skip this import</button></div></div>{error ? <p role="alert" className="break-words text-sm text-[var(--danger)]">{error}</p> : null}</div>;
}
...
export function SkipCoordinateButton({ importId, issueId }: { importId: string; issueId: string }) {
  ...
  return <div className="space-y-2"><button disabled={pending} className="rounded-lg border px-3 py-2" onClick={() => void skip()}>{pending ? "Skipping episode record…" : "Skip this episode record"}</button><p className="text-sm text-[var(--muted)]">This skips only this unmatched episode record. The show and all other successfully matched episode history can still be imported.</p>{message ? <p role="status" className="text-sm">{message}</p> : null}{error ? <p role="alert" className="text-sm text-[var(--danger)]">{error}</p> : null}</div>;
}
...
export function DeleteImportButton({ importId }: { importId: string }) {
  ...
  return <div className="space-y-2"><button disabled={pending} className="rounded-lg border px-3 py-2 text-[var(--danger)]" onClick={() => void remove()}>{pending ? "Deleting import…" : "Delete import session"}</button>{error ? <p role="alert" className="text-sm text-[var(--danger)]">{error}</p> : null}</div>;
}
...
export function ForgetTvTimeButton() {
  ...
  return <div className="space-y-2"><button disabled={pending} className="rounded-lg border px-3 py-2 text-[var(--danger)]" onClick={() => void forget()}>{pending ? "Forgetting TV Time data…" : "Forget all TV Time import data"}</button>{message ? <p role="status" className="text-sm">{message}</p> : null}{error ? <p role="alert" className="text-sm text-[var(--danger)]">{error}</p> : null}</div>;
}
