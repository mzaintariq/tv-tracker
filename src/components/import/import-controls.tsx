...
export function ResolutionControls({ importId, itemId, itemLabel = "this import item", candidates, candidateMetadata }: { importId: string; itemId: string; itemLabel?: string; candidates: number[]; candidateMetadata?: CandidateDisplay[] }) {
  ...
  return <div className="space-y-3"><div className="grid gap-3 sm:grid-cols-2">{candidatesForDisplay(candidates, candidateMetadata).map((candidate) => <CandidateCard key={candidate.id} candidate={candidate} disabled={pending} onUse={() => void run(() => confirmImportCandidate(importId, itemId, candidate.id))} />)}</div><div className="space-y-2"><label htmlFor={manualIdInput} className="block text-sm font-medium">Manual TMDB ID for {itemLabel}</label><p id={manualHelp} className="text-sm text-[var(--muted)]">Enter the numeric TMDB identifier when none of the suggested matches is correct.</p><div className="flex flex-wrap gap-2"><input id={manualIdInput} className="w-36 rounded-lg border px-3 py-2" inputMode="numeric" aria-describedby={manualHelp} placeholder="TMDB ID" value={manualId} onChange={(event) => setManualId(event.target.value)} /><button aria-label={`Confirm TMDB ID for ${itemLabel}`} disabled={pending || !/^\d+$/.test(manualId)} className="rounded-lg border px-3 py-2" onClick={() => void run(() => confirmImportCandidate(importId, itemId, Number(manualId)))}>Confirm ID</button><button aria-label={`Skip ${itemLabel} from this import`} disabled={pending} className="rounded-lg border px-3 py-2 text-[var(--danger)]" onClick={() => void run(() => skipImportItem(importId, itemId))}>Skip this import</button></div></div>{error ? <p role="alert" className="text-sm text-[var(--danger)]">{error}</p> : null}</div>;
}
