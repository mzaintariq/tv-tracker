...
async function mount(): Promise<ReactTestRenderer> {
  ...
  await act(() => {
    renderer = create(createElement(ImportUploadForm, { mode: "analyze" }));
  });
  if (!renderer) throw new Error("Renderer unavailable.");
  return renderer;
}
async function chooseFile(renderer: ReactTestRenderer): Promise<void> {
  await act(() =>
    renderer.root
      .findByType("input")
...
describe("ImportUploadForm response handling", () => {
  it("labels the required ZIP input and associates instructions", async () => {
    ...
    expect(input.props).toMatchObject({
      id: "tv-time-analyze-zip",
      required: true,
      "aria-describedby": "tv-time-analyze-zip-instructions",
    });
    expect(
  ...
  it("navigates after a valid Analyze success", async () => {
    vi.stubGlobal(
      ...
      vi
        .fn()
        .mockResolvedValue(
          new Response(JSON.stringify({ importId: "created" }), {
            status: 200,
          }),
        ),
    );
    const renderer = await mount();
    await chooseFile(renderer);
    expect(router.push).toHaveBeenCalledWith("/profile/import/created");
    ...
    vi.stubGlobal(
      ...
      vi
        ...
        .mockResolvedValue(
          new Response(
            JSON.stringify({
              code: "zip_compressed_too_large",
              error: "The ZIP exceeds 3.5 MB.",
            }),
            { status: 400 },
          ),
        ),
  ...
  it.each([
    ...
    "uses safe feedback for HTML, empty, and malformed responses",
    async (response) => {
      ...
      const message = renderer.root
        ...
        .children.join("");
      expect(message).toBe(
        "The import could not be uploaded. Please try again.",
      );
      expect(message).not.toContain("private proxy");
    },
  );
  ...
  it("clears pending and remains in place after network rejection", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockRejectedValue(new Error("raw network stack")),
    );
    const renderer = await mount();
    await chooseFile(renderer);
    expect(renderer.root.findByType("input").props.disabled).toBe(false);
