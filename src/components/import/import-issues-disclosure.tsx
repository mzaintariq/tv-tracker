...
function CoordinateText({ issue }: { issue: ImportIssueForDisplay }) {
  return <><p className="break-words font-medium">{issue.sourceTitle ?? "Unknown show"} — Season {issue.seasonNumber ?? "?"}, Episode {issue.episodeNumber ?? "?"}</p><p className="break-words text-sm text-[var(--muted)]">{issue.status === "resolved" ? "This episode was previously unavailable in TMDB metadata and was imported after metadata reconciliation." : "No matching TMDB episode was found. This individual history record will not be imported."}</p>{issue.seasonNumber === 0 ? <p className="break-words text-sm text-[var(--muted)]">Specials are also excluded from normal progress.</p> : null}</>;
}
...
function IssueRow({ importId, issue }: { importId: string; issue: ImportIssueForDisplay }) {
  ...
  return <article className="min-w-0 space-y-2 rounded-lg border p-3">
    {issue.issue_type === "missing_episode_coordinate" ? <CoordinateText issue={issue} /> : <>
      <p className="break-words font-medium">{issue.issue_type === "movie_favourites_confirmation" ? issueTypeLabel(issue.issue_type) : issue.sourceTitle ?? issueTypeLabel(issue.issue_type)}</p>
      <p className="break-words text-sm text-[var(--muted)]">{issue.mediaType ? `${issue.mediaType === "tv" ? "TV show" : "Movie"} · ` : ""}{issueStatusLabel(issue.status)}</p>
      {issue.issue_type === "movie_favourites_confirmation" && issue.status === "open" ? <><p className="break-words text-sm">TV Time’s export does not identify which cached movies were favourites. Choose whether to apply the importer’s possible movie-favourite records.</p><p className="break-words text-sm text-[var(--muted)]">This decision must be made before the import can be applied.</p></> : null}
      {issue.status === "resolved" && issue.selectedTmdbId ? <div className="flex min-w-0 flex-col gap-3 min-[360px]:flex-row min-[360px]:items-center"><div className="relative aspect-[2/3] h-[69px] w-[46px] max-w-full shrink-0 overflow-hidden rounded bg-[var(--surface-elevated)]"><MediaPoster source={issue.selectedCandidate?.posterPath} title={issue.selectedCandidate?.title ?? issue.sourceTitle ?? ""} alt="" sizes="46px" tmdbSize="w92" fallbackLabel="No poster" fallbackClassName="px-1 text-center text-[10px] text-[var(--muted)]" /></div><p className="min-w-0 break-words text-sm">Matched to {issue.selectedCandidate?.title ?? "TMDB title"}{issue.selectedCandidate?.year ? ` (${issue.selectedCandidate.year})` : ""} · TMDB {issue.selectedTmdbId}</p></div> : null}
      {issue.status === "skipped" && issue.mediaType ? <p className="break-words text-sm">{issue.mediaType === "tv" ? "TV show" : "Movie"} · Skipped from this import</p> : null}
    </>}
...
export function ImportIssuesDisclosure({ importId, issues, initialExpanded, initialCompletedExpanded = false, initialExpandedGroups = [] }: { importId: string; issues: ImportIssueForDisplay[]; initialExpanded?: boolean; initialCompletedExpanded?: boolean; initialExpandedGroups?: string[] }) {
  ...
  return <section className="min-w-0 rounded-xl border p-4"><DisclosureButton expanded={expanded} controls="import-issues-content" onClick={() => setExpanded((value) => !value)}><span className="font-semibold">Decisions and issues ({model.total})<span className="mt-1 block text-sm font-normal text-[var(--muted)]">{model.openBlocking} open blocking · {model.openNonBlocking} informational · {model.resolved} resolved · {model.skipped} skipped</span></span></DisclosureButton>{expanded ? <div id="import-issues-content" className="mt-4 min-w-0 space-y-4">
    {model.needsAttention.length ? <section className="min-w-0 rounded-lg border p-3"><DisclosureButton expanded={attention} controls="issues-attention" onClick={() => setAttention((value) => !value)}><b>Needs attention ({model.needsAttention.length})</b></DisclosureButton>{attention ? <div id="issues-attention" className="mt-3 space-y-3">{model.needsAttention.map((issue) => <IssueRow key={issue.id} importId={importId} issue={issue} />)}</div> : null}</section> : null}
    {model.notices.length ? <section className="min-w-0 space-y-2 rounded-lg border p-3"><h3 className="break-words font-medium">Information and notices ({model.notices.length})</h3>{model.notices.map((issue) => <Notice key={issue.id} issue={issue} />)}</section> : null}
    {model.completedGroups.length ? <section className="min-w-0 rounded-lg border p-3"><DisclosureButton expanded={completed} controls="issues-completed" onClick={() => setCompleted((value) => !value)}><b>Resolved or skipped ({model.resolved + model.skipped})</b></DisclosureButton>{completed ? <div id="issues-completed" className="mt-3 space-y-2">{model.completedGroups.map((group, index) => { const open = groups.has(group.key); const controls = `completed-group-${index}`; return <section key={group.key} className="min-w-0 rounded-lg border p-3">{group.expandable ? <DisclosureButton expanded={open} controls={controls} onClick={() => toggle(group.key)}><span>{group.label} — {group.issues.length}</span></DisclosureButton> : <p className="break-words">{group.label} — {group.issues.length}</p>}{group.key === "coordinates:skipped" ? <p className="mt-2 break-words text-sm text-[var(--muted)]">The show and all other successfully matched episode history will still be imported.</p> : null}{group.expandable && open ? <div id={controls} className="mt-3 space-y-2">{group.key === "coordinates:skipped" ? <CoordinateNoticeGroups importId={importId} issues={group.issues} groups={groups} toggle={toggle} /> : group.issues.map((issue) => <IssueRow key={issue.id} importId={importId} issue={issue} />)}</div> : null}</section>; })}</div> : null}</section> : null}
  </div> : null}</section>;
}
