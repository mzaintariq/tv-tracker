...
vi.mock("@/components/movies/movie-card", () => ({
  MovieCard: ({
    ...
    createElement(
      "span",
      { "data-movie-title": movie.media.title },
      movie.media.title,
      action,
    ),
}));
...
vi.mock("@/components/movies/quick-mark-movie-watched", () => ({
  QuickMarkMovieWatched: ({ title }: { title: string }) =>
    createElement(
      "button",
      { type: "button", "aria-label": `Mark ${title} watched` },
      "Mark watched",
    ),
}));
...
describe("limited movie sections", () => {
  it.each(["Watch Next", "Watched", "Upcoming"] as const)(
    ...
    async (title) => {
      ...
      const expected = Array.from(
        { length: 12 },
        (_, index) => `Movie ${String(index + 1).padStart(2, "0")}`,
      );
      const visibleTitles = () =>
        renderer.root
          .findAll((node) => typeof node.props["data-movie-title"] === "string")
          .map((node) => node.props["data-movie-title"] as string);
      ...
      const toggle = renderer.root
        ...
        .find((button) => button.children.includes("Show all 12"));
  ...
  it.each([8, 10])(
    "does not render a disclosure for %s movies",
    async (count) => {
      const renderer = await renderLimitedSection("Watched", count);
      expect(
        renderer.root.findAll(
          (node) => typeof node.props["data-movie-title"] === "string",
        ),
      ).toHaveLength(count);
