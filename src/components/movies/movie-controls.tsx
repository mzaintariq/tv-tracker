...
function Message({ result }: { result: MovieActionResult | null }) {
  return result ? (
    <div className="min-w-0 break-words text-sm">
      {result.error ? (
        <p id="movie-date-error" className="text-[var(--danger)]" role="alert">
          {result.error}
        </p>
      ) : null}
      {result.success ? (
...
export function MovieControls({
  ...
  const run = (task: () => Promise<MovieActionResult>) =>
    start(async () => {
    ...
    <section className="min-w-0 space-y-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-5">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:flex-wrap">
        <button
          ...
          disabled={pending}
          onClick={() =>
            run(() => setMovieWatched(tmdbId, mediaId, !membership.watched_at))
          }
        >
          {pending
            ? "Saving…"
        ...
        <div className="flex w-full min-w-0 max-w-full flex-col gap-2 sm:flex-row sm:flex-wrap sm:items-end">
          ...
          <div className="w-full min-w-0 max-w-full sm:w-auto">
            <input
              ...
              aria-describedby={result?.error ? "movie-date-error" : undefined}
          ...
          <button
            aria-label="Save movie watched date"
            className={button}
            disabled={pending}
            onClick={() => {
              const input = document.getElementById("movie-watched-date");
              if (input instanceof HTMLInputElement)
                run(() => updateMovieWatchedAt(tmdbId, mediaId, input.value));
            }}
          >
            Save date
          </button>
      ...
      <div className="min-w-0">
        <button
          className={`${button} text-[var(--danger)]`}
          disabled={pending}
          onClick={() => {
            if (
              window.confirm(
                "Remove this movie? Its watched date and favourite state will be permanently deleted.",
              )
            )
              start(async () => {
                const response = await removeMovie(tmdbId, mediaId);
                setResult(response);
                if (!response.error) router.push("/movies");
              });
          }}
        >
          Remove from library
