...
function Message({ result }: { result: MovieActionResult | null }) { return result ? <div className="text-sm">{result.error ? <p id="movie-date-error" className="text-[var(--danger)]" role="alert">{result.error}</p> : null}{result.success ? <p className="text-[var(--success)]" role="status">{result.success}</p> : null}</div> : null; }
...
export function MovieControls({ tmdbId, mediaId, membership }: { tmdbId: number; mediaId: string; membership: UserMovie }) {
  ...
  return <section className="space-y-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5">
    ...
    {membership.watched_at ? <div className="flex flex-wrap items-end gap-2"><label htmlFor="movie-watched-date" className="text-sm">Watched date</label><input id="movie-watched-date" type="datetime-local" defaultValue={dateValue} max={new Date().toISOString().slice(0, 16)} aria-describedby={result?.error ? "movie-date-error" : undefined} aria-invalid={result?.error ? true : undefined} className="mt-1 block rounded-lg border border-[var(--border)] bg-[var(--surface)] p-2" /><button aria-label="Save movie watched date" className={button} disabled={pending} onClick={() => { const input = document.getElementById("movie-watched-date"); if (input instanceof HTMLInputElement) run(() => updateMovieWatchedAt(tmdbId, mediaId, input.value)); }}>Save date</button></div> : null}
