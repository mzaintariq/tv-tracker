...
export function MovieCard({ movie, action }: { movie: MovieSnapshot<MovieLibraryMedia>; action?: React.ReactNode }) {
  return <article className="relative min-w-0">
    <Link href={`/movies/${movie.media.tmdb_id}`} className="poster-interactive-surface block min-w-0 overflow-hidden rounded-xl border bg-[var(--surface)]" aria-label={`${movie.media.title}${movie.membership.is_favourite ? ", favourite" : ""}`}>
