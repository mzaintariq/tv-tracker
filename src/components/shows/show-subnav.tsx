  ...
  return (
    <div className="sticky top-[calc(0.75rem+var(--safe-area-top))] z-30 flex justify-center pb-1">
      ...
      >
        <span
          aria-hidden="true"
          className={`pointer-events-none absolute bottom-1 left-1 top-1 w-[calc(50%-0.25rem)] rounded-full bg-[var(--accent)] shadow-sm transition-transform duration-200 ease-out ${selected === "upcoming" ? "translate-x-full" : "translate-x-0"}`}
        />
        {items.map((item) => (
          <Link
            key={item.href}
