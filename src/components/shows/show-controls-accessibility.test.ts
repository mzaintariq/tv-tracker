...
async function mount(element: React.ReactElement): Promise<ReactTestRenderer> {
  ...
  await act(() => {
    renderer = create(createElement(NotificationProvider, null, element));
  });
...
describe("show control accessibility", () => {
  it("presents tracking status as one named three-way toggle", async () => {
    ...
    expect(
      toggles.map((toggle) => toggle.findByType("span").children.join("")),
    ).toEqual(["Active", "Paused", "Dropped"]);
  ...
  it("uses a native named radio group and labelled mode-specific controls", async () => {
    const renderer = await mount(
      createElement(InitialProgressForm, {
        tmdbId: 42,
    ...
    expect(new Set(radios.map((radio) => radio.props.name))).toEqual(
      new Set(["progress-mode-42"]),
    );
    expect(
      renderer.root.findAllByType("fieldset")[0].findByType("legend").children,
    ).toContain("Starting progress");

    await act(() => radios[1].props.onChange());
    const select = renderer.root.findByType("select");
    expect(select.props.id).toBe("first-unwatched-42");
    expect(
      renderer.root.findByProps({ htmlFor: select.props.id }).children,
    ).toContain("Select the first unwatched episode");

    await act(() =>
      renderer.root.findAllByProps({ type: "radio" })[2].props.onChange(),
    );
    const fieldsets = renderer.root.findAllByType("fieldset");
    expect(fieldsets).toHaveLength(2);
    expect(fieldsets[1].findByType("legend").children).toContain(
  ...
  it("gives repeated episode date controls contextual names and error hooks", async () => {
    ...
    expect(
      renderer.root.findByProps({ htmlFor: input.props.id }).children.join(""),
    ).toContain("S02E04");
    expect(
      renderer.root.findByProps({
        "aria-label": "Save watched date for S02E04: Episode 4",
      }),
    ).toBeDefined();
  });
});
