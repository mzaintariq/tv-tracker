...
insert into public.media_items(id,tmdb_id,media_type,title,genres,vote_average) values
('28000000-0000-0000-0000-000000000001',801,'tv','TV fixture','[]',null),
('28000000-0000-0000-0000-000000000002',802,'movie','Movie fixture','[{"id":18,"name":"Drama"}]',7.8);
...
select is(public.load_movie_library_data()->'movies'->0->'media'->'genres','[{"id": 18, "name": "Drama"}]'::jsonb,'movie genres are projected');
select is((public.load_movie_library_data()->'movies'->0->'media'->>'vote_average')::numeric,7.8::numeric,'movie rating is projected');
select * from finish();
rollback;
