...
language sql stable security invoker set search_path = '' as $$
  with owned as (
    select um.id membership_id, m.id media_item_id, m.tmdb_id, m.title,
      m.poster_path, um.watched_at, um.is_favourite, m.release_dates_synced_at
  ...
  from selected s
  where s.theatrical_date >= p_today - 30 or s.digital_date >= p_today - 30
    or (s.theatrical_date is null and s.digital_date is null)
  order by
    case
    ...
    case when not (s.theatrical_date > p_today or s.digital_date > p_today) then
      greatest(
...
create or replace function public.load_movie_upcoming_refresh_candidates(p_tmdb_ids integer[])
returns table (tmdb_id integer, release_dates_synced_at timestamptz)
language sql stable security invoker set search_path = '' as $$
  select m.tmdb_id, m.release_dates_synced_at
  from public.user_movies um
  join public.media_items m on m.id = um.media_item_id and m.media_type = 'movie'
  where um.user_id = auth.uid() and m.tmdb_id = any(coalesce(p_tmdb_ids, '{}'))
  order by m.tmdb_id;
$$;

revoke execute on function public.load_movie_upcoming_refresh_candidates(integer[]) from public, anon, service_role;
grant execute on function public.load_movie_upcoming_refresh_candidates(integer[]) to authenticated;
